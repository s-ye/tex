...
The contraction theorem we saw fits into a general three-step strategy in deformation theory:
\begin{enumerate}
    \item
Construct a formal solution by solving the problem on all infinitesimal thickenings.
\item Algebraize the formal object to obtain an honest scheme over a complete local base.
\item Descend from the complete local base to a finite type base, obtaining an algebraic space or stack.
\end{enumerate}
...
\begin{itemize}
    ...
    Then both ideals contain an ideal of definition of $\widehat{X}'$. This roughly says that $\widehat{f}$ is formally \textbf{étale} and \textbf{locally a complete intersection} outside $Y'$.
    ...
    \]
    \item \textbf{Formal surjectivity outside $Y$. (Valuative criterion for formal modifications.)}
