...
\]
If our bundle \(F\) comes from an \(\mathrm{SL}_r\)-bundle, \emph{i.e.}\ has trivial determinant,
...
\emph{special complexes}, that the
$\bigwedge^{\mathrm{max}} K_i^*$ glue together over $S$ and define a canonical
square root of $\mathcal D_F$, called the \emph{pfaffian bundle}.
...
In nature, representations of \(L\mathfrak{g}\) are projective; this is why we look at (true) representations of \(\widehat{L\mathfrak{g}}\). Fix an integer \(\ell\). Call a representation of \(\widehat{L\mathfrak{g}}\) \emph{of level} \(\ell\), if the center acts by multiplication by \(\ell\).
...
Such \(\widehat{L\mathfrak{g}}\)-modules are called \emph{integrable},
...
\]
$P_\ell$ is called the set of \emph{integrable weights} of level \(\ell\). It indexes the irreducible integrable \(\widehat{L\mathfrak{g}}\)-modules of level \(\ell\).
...
the representation \(\bar{\pi}\) may be \emph{integrated} to a (unique)
...
\emph{canonical central extension} of \(L \tilde{G}\):
\[
...
we can \emph{normalize} the automorphism \(u\) of Proposition~7.3.2.
...
Let \(G\) be a connected simple complex group and \(X\) be a connected smooth projective complex curve. Consider the canonical central extension (\ref{canonical-central-extension-gps}) and form the affine Grassmannian \(\mathcal{Q}_{\widetilde{G}} = \widehat{L\widetilde{G}} / \widehat{L^+\widetilde{G}}\). By Lemma \ref{lemma-splitting-over-l-plus-g}, the central extension splits over \(L^+\widetilde{G}\) so we get a character \begin{align*}
...
\begin{proposition}[Pullback to \(\mathbb{P}^1_\mathbb{C}\)]
    \leavevmode
\begin{enumerate}[(i)]
\item The pullback defines an isomorphism
\[
...
\begin{theorem}
The restriction of (\ref{canonical-central-extension-gps}) to \(L_X G\) splits.
\end{theorem}

\begin{proof}
Consider the inclusion \(i : L_X \widehat{G} \hookrightarrow L\widehat{G}\).
...
By the residue theorem, the cocycle~(7.2a) is trivial on \(L_X \mathfrak{g}\),
so \(L_X \mathfrak{g}\) embeds as a subalgebra of \(\widehat{L\mathfrak{g}}\).
...
The key point is that \(B \neq 0\) by the theory of conformal blocks. From the commutativity of~(7.3a), for \(\gamma \in L_X G(R)\),
the automorphism \(u_\gamma\) of \(H\) maps coinvariants to coinvariants,
giving a morphism of \(\mathbb{C}\)-groups
\[
\pi : L_X G \longrightarrow \mathrm{PGL}(B),
\]
and a commutative diagram
\[
\begin{tikzcd}
1 \arrow[r] & \mathbb{G}_m \arrow[r] & \widehat{L_X G} \arrow[r] & L_X \widehat{G} \arrow[r] & 1 \\
...
As \(L_X \tilde{G}\) is an \emph{integral} ind-group by Proposition~9.1.1,
it follows that \(\pi\) has to be the constant map identity.
...
\subsection*{10.5. The case of the special linear group.}
...
\begin{proof}
Consider the morphism \(\overline{\varphi}\) of~10.2.1:
\[
\begin{tikzcd}
...
By definition of \(\varphi\), it suffices to treat the rank-two case,
where the family corresponds to the inclusion
\[
W =
\begin{pmatrix}
d & cz^{-1} \\ bz & a
...
\mathbb{C}((z)) \oplus \mathbb{C}((z)).
\]
The lattice \(V = z^{-1}\mathbb{C}[[z]] \oplus \mathbb{C}[[z]]\)
defines the rank-two bundle \(F = \mathcal{O}_X(p) \oplus \mathcal{O}_X\).
Via the inclusion \(W \subset V\),
the family \(E_{[a:c]}\) is the kernel of
\(F \to \mathbb{C}_p\)
sending local sections \((z^{-1}f,g)\) to \(a f(p) - c g(p)\).
Hence
\[
\mathcal{D}_E = \mathcal{O}_{\mathbb{P}^1_\mathbb{C}}(1)
\]
\end{proof}
