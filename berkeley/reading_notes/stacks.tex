...
\section{GIT and moduli of semistable bundles}
...
\begin{theorem}[Moduli of rank 2 vector bundles with fixed determinant]
...
\in \Alt_N(H^0(L))
\]
will be called the \emph{Gieseker matrix}, or \emph{Gieseker point}, 
...
\emph{$H^0$-semistable} (resp.\ \emph{$H^0$-stable}).
\end{definition}
...
violating $H^0$-semistability. 
So the image has rank~$2$.
...
We are now prepared to prove Theorem 10.1. For this we need to study the
