...
\begin{definition}
    A Cartan matrix $A$ is called \textbf{symmetrizable} if there exists a diagonal matrix $D = \mathrm{diag}(\varepsilon_1,\dots,\varepsilon_n)$ with positive entries $\varepsilon_i$ such that $DA$ is symmetric.
\end{definition}
...
\begin{proof}
    ...
    The theorem now follows from Theorem 5.4.
\end{proof}

It follows from Proposition 4.7 that if $\alpha$ is a \emph{null-root}, i.e.\
...
\subsection*{Translations in the Affine Weyl Group}
...
\begin{equation}\label{eq:ta}
    ...
    = \lambda + \langle \lambda, K \rangle \alpha 
      - \big( (\langle \lambda,\alpha\rangle 
      + \tfrac{1}{2}|\alpha|^2 \langle \lambda, K \rangle ) \big)\delta.
\end{equation}
\begin{remark}
    Kac writes that this quadratic term which appears in the definition of $t_\alpha$ "explains" the appearance of theta functions in the theory of affine algebras. This is because when you compute the characters of highest-weight representations of affine Kac-Moody algebras, you sum over the affine Weyl group:
...
\noindent
Now we can easily deduce the additivity property of $t_\alpha$:
\begin{equation}
    t_\alpha t_\beta = t_{\alpha+\beta}.
\end{equation}
Indeed, by the previous formula it is sufficient to check this 
for $\lambda = \Lambda_0$. Using the above, we compute
\begin{align*}
    t_\alpha t_\beta (\Lambda_0) 
      &= t_\alpha(\Lambda_0 + \beta - \tfrac{1}{2}|\beta|^2 \delta) \\
      &= \Lambda_0 + \alpha - \tfrac{1}{2}|\alpha|^2 \delta
         + \beta - \tfrac{1}{2}|\beta|^2 \delta
         - \big((\beta - \tfrac{1}{2}|\beta|^2\delta | \alpha)\big)\delta \\
      &= \Lambda_0 + \alpha + \beta - \tfrac{1}{2}|\alpha+\beta|^2 \delta \\
      &= t_{\alpha+\beta}(\Lambda_0).
\end{align*}

\noindent
We also have the compatibility with the finite Weyl group:
\begin{equation}
    t_{w(\alpha)} = w t_\alpha w^{-1}, \qquad w \in \dot{W}.
\end{equation}
Indeed,
\[
...
\]
and the claim follows since $w(K)=K$ and $(\cdot|\cdot)$ is $\dot{W}$-invariant.

\medskip

\noindent
Now we introduce the following important lattice $M \subset \mathfrak{h}^*$.
Let $\mathbb{Z}(\dot{W}\cdot \theta^\vee)$ denote the lattice in $\mathfrak{h}^*$
spanned over $\mathbb{Z}$ by the (finite) set $\dot{W}\cdot \theta^\vee$, and set
\[
    M = \nu(\mathbb{Z}(\dot{W}\cdot \theta^\vee)).
\]
Here is a description of the lattice $M$:
\begin{equation}
    M = Q = Q^\vee \quad \text{if $A$ is symmetric or $r > a_0$};
\end{equation}
\begin{equation}
    M = \nu(Q^\vee) = \nu(Q^\vee) \quad \text{otherwise}.
\end{equation}

\noindent
Indeed, if $r=1$, then $\theta^\vee$ is a short root of $\Delta^\vee$, hence
$\dot{W}\cdot \theta^\vee = (\Delta^\vee)_s$. In the finite type case the root
lattice is spanned over $\mathbb{Z}$ by the short roots, hence 
$M = \nu(Q^\vee)$, as claimed. Equivalently,
\[
    M = Q^\vee \quad (\text{resp.\ } M = \mathbb{Z}\Delta_l)
    \quad \text{if $r=1$ and $A$ is symmetric (resp.\ nonsymmetric).}
\]
Similarly, if $a_0 r = 2$ or $3$, then $\theta^\vee$ is a long root, hence 
$\dot{W}\cdot \theta^\vee = (\Delta^\vee)_l$, and we get $M=Q^\vee$.
Finally, for $A_{2\ell}^{(2)}$ one has $\nu(\theta^\vee) = \tfrac{1}{2}\theta$, 
hence $M=\tfrac{1}{2}\mathbb{Z}\Delta_\ell$, which again agrees with the above.
...
$\mathfrak{h}^*$ by formula \eqref{eq:ta}. We denote the corresponding subgroup
of $GL(\mathfrak{h}^*)$ by $T$, and call it the \emph{group of translations}.
\medskip

\begin{proposition}
    We have $W = \dot{W} \ltimes T$.
\end{proposition}

\begin{proof}
By the above, $t_\alpha \in W$ for $\alpha \in \nu(\theta^\vee)$, and 
$t_{w(\alpha)} \in W$ for $w \in \dot{W}$. 
Now it follows from additivity that $t_\alpha \in W$ for all $\alpha \in M$. 
Since $\dot{W}$ is finite and $M$ is a free abelian group, 
we have $\dot{W}\cap T = 1$. By conjugation invariance, $T$ is a normal 
subgroup of $W$. Finally, the subgroup of $W$ generated by $T$ and $\dot{W}$ 
contains all simple reflections, hence coincides with $W$.
\end{proof}

\subsection*{Big Picture Remarks}
The operators $t_\alpha$ are the \emph{translations} in the affine Weyl group. 
They commute with each other and form a free abelian group $T$ generated by 
a lattice $M$ (essentially a coroot lattice, depending on the type).
The finite Weyl group $\dot{W}$ normalizes $T$, so the full affine Weyl group 
splits as a semidirect product:
\[
    W \;\cong\; \dot{W} \ltimes T.
\]
Thus, the affine Weyl group is ``finite reflections + translations.’’
This explains why it is infinite (because of the translation subgroup), 
and why it still has a very rigid structure, acting as symmetries of the 
affine root system or tessellations of Euclidean space.
