...
\red{Where does one invoke the fact that we quotiented out by the maximal ideal intersecting $\mathfrak h$ trivially, when deriving the Serre relations?}

\red{How to interpret this number $\langle \lambda, \alpha_i^\vee \rangle$? I feel like I see it a lot.}
...
We continue with another perspective of affine Lie algebras.

\subsection*{Central extensions of $\mathfrak{g}[z,z^{-1}]$}

Let $\mathfrak{g}$ be a simple Lie algebra over $\mathbb{C}$, e.g.\ $\mathfrak{sl}_n$, $\mathfrak{so}_n$ for $n \geq 5$, $\mathfrak{sp}_{2n}$ for $n \geq 4$, exceptional ones, etc.

Fix an invariant inner product $\langle\ ,\ \rangle$ on $\mathfrak{g}$ satisfying
\[
...
\section{Questions (Sept 17)}
\begin{enumerate}[A.]
    \item I understand how to build the loop algebra $L\mathfrak{g}$, but why exactly do we need to pass to its central extension, and then further adjoin the derivation $d$? What breaks if we just work with $L\mathfrak{g}$ itself, or with the central extension $\widehat{\mathfrak{g}'}$?
    \item Is there a precise sense in which the Casimir operator corresponds to the Laplacian on the loop group, as the finite-dimensional Casimir corresponds to the Laplacian on the compact group?
    \item The invariant bilinear form often shows up in the definition of line bundles on affine Grassmannians (via central extensions). How should I see this connection starting from the algebraic side?
    \item In the representation theory of affine Lie algebras, characters are related to modular forms. How early can one see hints of this modularity from the perspective of Lie algebra cohomology or the generalized Casimir?
    \item Where does the level (the eigenvalue of the central element) start to matter in representation theory? Is it already implicit in the structure of the invariant bilinear form?
\end{enumerate}
