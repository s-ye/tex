...
As usual, we care about \emph{central extensions} of this Lie algebra.

Recall that central extensions are classified by $H^2(\mathfrak{g}[z,z^{-1}])$, so we want to compute this cohomology group. We have the Chevalley complex
\[
...
\]
Let $\omega\in C^2(L)$ be $\mathfrak g$-invariant. Fix degrees $k,\ell$. The bilinear map
\[
...
This still consists of infinitely many parameters. We will whittle it down.
...
\begin{definition}[Affine Kac-Moody Lie algebra]
...
Equivalently,
\[
[x(z), y(z)]_{\widehat{\mathfrak{g}}} = [x(z), y(z)]_{\mathfrak{g}} + \operatorname{Res}_{z=0}(x(z), dy(z)) \cdot c.
\]

Therefore, we define the \textbf{(untwisted) affine Kac--Moody Lie algebra} $\widehat{\mathfrak{g}}$ associated to a simple (finite-dimensional) Lie algebra $\mathfrak{g}$ to be the unique (up to rescaling) nontrivial central extension of the loop space $\mathfrak{g}[z,z^{-1}]$.
\end{definition}

\red{Is there a way, given a generalized Cartan matrix of affine type, to see that it comes from a simple Lie algebra $\mathfrak{g}$?}
...
Then $\widehat{\mathfrak{sl}}_2$ is generated by $h, c, e, f, fz, ez^{-1}$. 
\end{example}
