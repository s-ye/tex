...
The bounded derived category of coherent sheaves $D^{b}(X)$ is a powerful invariant of an algebraic variety. At first sight, it appears to be merely a technical enlargement of the abelian category $\Coh(X)$: one takes complexes of coherent sheaves, identifies quasi-isomorphic complexes, and equips the result with the structure of a triangulated category.

Several basic geometric operations—tensoring with line bundles, taking pullbacks and pushforwards under morphisms, forming extensions—are naturally expressed at the level of complexes. Passing to the derived category packages these operations into a single formal framework in which cohomology, duality, and spectral sequences are built in from the outset.

A natural question, then, is the extent to which the derived category determines the underlying variety. In general, $D^{b}(X)$ is a far more flexible object than $X$ itself: there exist derived equivalences between varieties that are not isomorphic. However, in the presence of positivity of the canonical class, the situation is dramatically different. Bondal and Orlov proved in \cite{BondalOrlov2001} that for a smooth projective variety $X$ with ample (or anti-ample) canonical bundle $\omega_X$, the derived category $D^{b}(X)$ determines $X$ uniquely up to isomorphism.
...
Finally form $D(\mathcal{A})$ by formally inverting all quasi-isomorphisms in $K(\mathcal{A})$. 
The morphisms in $D(\mathcal{A})$ are a little subtle. 
One cannot obtain $D(\mathcal{A})$ by staying inside $K(\mathcal{A})$ and demanding that every 
quasi-isomorphism already admit an inverse there. 
If $X$ is not injective in $\mathcal{A}$, the inclusion $i\colon X[0]\to I^\bullet$ into an injective resolution 
is a quasi-isomorphism, but in general there is no morphism $p\colon I^\bullet\to X[0]$ in $K(\mathcal{A})$ with 
\[
[p]\circ[i]=[\mathrm{id}_{X[0]}],\qquad [i]\circ[p]=[\mathrm{id}_{I^\bullet}].
\]
Requiring such equalities in $K(\mathcal{A})$ would force $i$ to be a homotopy equivalence, which is too strong because not every quasi-isomorphism is a homotopy equivalence.

Instead one passes to the localization
\[
D(\mathcal{A}) = K(\mathcal{A})[S^{-1}],
\]
a new category characterized by the universal property that the localization functor
\[
Q\colon K(\mathcal{A})\longrightarrow D(\mathcal{A})
\]
sends every quasi-isomorphism to an isomorphism and is universal with this property. 
Concretely, morphisms in $D(\mathcal{A})$ can be represented by “roofs”
\[
X \xleftarrow{s} X' \xrightarrow{f} Y
\]
with $s$ a quasi-isomorphism, which encodes the formal composite $f\circ s^{-1}$ in the localized category.
...
First we will show that $s=n$. Write $t = s - n$.
From the Serre functor formula, we have
\[P \otimes \omega_X [n] \simeq P[s] \implies P \otimes \omega_X \simeq P[t]\]
Taking cohomology sheaves gives
\[\cH^i(P\otimes\omega_X) \cong \cH^i(P)\otimes\omega_X \cong \cH^{i+t}(P)\]
where the first equality follows because tensoring with an invertible sheaf is an exact functor on the abelian category of coherent sheaves and the second equality follows from the shift by $[t]$ on the right-hand side. 
...
It is a consequence of general theory
  of modules over local rings that for any two nonzero finite--length modules
  over a local ring $\cO_{X,x}$, there exists a nontrivial homomorphism
  between them. If $(R,\mathfrak{m})$ is a local ring and $M,N$ are finite--length $R$-modules, then note that any simple $R$-module is isomorphic to the residue field $k = R/\mathfrak{m}$. $M$ admits a quotient map onto a simple module $M \twoheadrightarrow k$ because $M$ being finite length, it is Noetherian and therefore admits a maximal submodule whose quotient is simple, hence isomorphic to $k$. Dually, $N$ admits an injection of a simple module $k \hookrightarrow N$ because $N$ being finite length, it is Artinian and therefore admits a minimal nonzero submodule which is necessarily simple, hence isomorphic to $k$. Then there is a map $M \to N$ passing through $k$.
...
\]
General theory in commutative algebra states that any finite module $M$ over an arbitrary noetherian local ring $(A,\mathfrak m)$ with $\Ext^{1}_{A}(M, A/\mathfrak m)=0$ is free.
...
F(\omega_{X}^{k})
 ...
 = \omega_{Y}^{k}.
\]
Using that $F$ is fully faithful, we conclude from this that for all $k$,
\[
H^{0}(X,\omega_{X}^{k})
...
\]
We first prove that point like objects in $D^b(Y)$ are of the form $k(y)[m]$ without assuming any positivity of $\omega_Y$.
...
The sheaf cohomology of $k(y)$ is clearly
\[
...
separates tangent directions if $\omega_{X}^{k}$ does.
\end{proof}
