...
\item Now reverse the ordered basis of $\C^n$ from
\[
e_1,\dots,e_n
\quad\text{to}\quad
e_n,\dots,e_1.
\]
This gives a second Schubert cell decomposition of $\Gr_{\C}(m;n)$.
If $\sigma=(\sigma_1,\dots,\sigma_n)$, let
\[
\sigma^\vee=(\sigma_n,\dots,\sigma_1)
\]
be the reversed binary string. Then the Schubert cell for the reversed flag indexed by $\sigma$ is the same geometric set as the Schubert cell for the original flag indexed by $\sigma^\vee$.

The point is that the two cell decompositions are related by the linear automorphism
\[
R:\C^n\to\C^n,\qquad e_i\mapsto e_{n+1-i}.
\]
This induces a homeomorphism
\[
R:\Gr_{\C}(m;n)\to \Gr_{\C}(m;n)
\]
sending the Schubert decomposition for the standard flag to the Schubert decomposition for the reversed flag. Hence corresponding cells in the two decompositions represent the same homology classes up to permutation.

More concretely, in each real dimension there is exactly one Schubert cell for each binary string of the corresponding inversion number, and the two decompositions are carried to one another by a homeomorphism of the Grassmannian. Therefore the two cell decompositions determine the same cellular homology basis.
...
\[
A:=H^*(X;\Z),\qquad
R:=A[x_1,\dots,x_r]\Big/\Big(\prod_i(1+x_i)=c_\bullet(V)\Big)
\cong H^*(\Fl(V);\Z).
\]
Write
\[
...
\noindent\textbf{(1) \(p_*\) is \(A\)-linear.}
Since \(p:\Fl(V)\to X\) is an oriented fiber bundle with compact fiber \(\Fl(\C^r)\),
integration along the fiber satisfies the projection formula
\[
p_*\bigl(p^*a\smile \alpha\bigr)=a\smile p_*(\alpha),
\qquad a\in A,\ \alpha\in H^*(\Fl(V)).
\]
Thus \(p_*\) is \(A\)-linear.

\medskip

\noindent\textbf{(2) On each fiber, \(\Delta\) integrates to \(r!\).}
On a fiber \(F=\Fl(\C^r)\), the classes \(x_i\) restrict to the Chern roots of the tautological splitting, and
\[
H^*(F;\Z)\cong \Z[x_1,\dots,x_r]/(e_1,\dots,e_r),
\]
where \(e_i\) are the elementary symmetric polynomials. The top cohomology \(H^{2\binom r2}(F;\Z)\cong \Z\) is generated by the class of
\[
\prod_{i=1}^r x_i^{r-i}.
\]
Now
...
\Delta=\prod_{i<j}(x_i-x_j)
\]
is alternating, and its highest monomial is
\[
x_1^{r-1}x_2^{r-2}\cdots x_{r-1}
\]
with coefficient \(1\). Since every permutation of the \(x_i\) induces a diffeomorphism of the fiber, and permuting two adjacent line bundles reverses the complex orientation of the fiber by a sign \(-1\), each term in the expansion of \(\Delta\) contributes the same fiber integral, namely \(1\). There are \(r!\) terms, so
\[
\int_F \Delta = r!.
\]
Equivalently,
...
Now define
\[
T(f):=\Delta^{-1}\sum_{\sigma\in S_r}\varepsilon(\sigma)\,
f(x_{\sigma(1)},\dots,x_{\sigma(r)}).
\]
We claim that \(T(f)\in A\), and that \(T=p_*\).

Indeed, the numerator
\[
\Alt(f):=\sum_{\sigma\in S_r}\varepsilon(\sigma)\,
f(x_{\sigma(1)},\dots,x_{\sigma(r)})
\]
is alternating in the \(x_i\). Hence it is divisible by \(\Delta\), so \(T(f)\) is a symmetric polynomial in the \(x_i\). But in the cohomology ring \(R\), every symmetric polynomial in the \(x_i\) can be expressed in terms of the elementary symmetric polynomials, which are the Chern classes \(c_i(V)\in A\). Therefore \(T(f)\in A\), as claimed.

Also, \(T\) is \(A\)-linear, since antisymmetrization is \(A\)-linear.

It remains to show that \(T\) and \(p_*\) agree. Since both are \(A\)-linear maps
\[
R\to A,
\]
it suffices to compare them on each fiber. Thus we may reduce to the case \(X=\mathrm{pt}\), where
\[
R=H^*(\Fl(\C^r);\Z).
\]

In this case, \(T\) vanishes on every class of degree \(<2\binom r2\), because antisymmetrization raises divisibility by \(\Delta\), so the quotient has degree strictly less than top degree unless \(f\) already contains the top alternating part. Thus \(T\) is an \(A\)-linear functional supported in top degree, just like \(p_*\).
...
=
\Delta^{-1}\sum_{\sigma\in S_r}\varepsilon(\sigma)\,
\Delta(x_{\sigma(1)},\dots,x_{\sigma(r)}).
\]
Since \(\Delta\) is alternating,
\[
\Delta(x_{\sigma(1)},\dots,x_{\sigma(r)})=\varepsilon(\sigma)\Delta,
\]
so
\[
T(\Delta)=\Delta^{-1}\sum_{\sigma\in S_r}\varepsilon(\sigma)^2\Delta
=\Delta^{-1}\sum_{\sigma\in S_r}\Delta
=r!.
\]
But we already saw that \(p_*(\Delta)=r!\). Since top cohomology of the fiber is rank one, this determines the fiber integration map uniquely. Hence
\[
T(f)=p_*(f)
\]
for all \(f\in R\).
