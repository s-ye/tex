...
A point is \textbf{special} if it is a node or a marked point.  Special points
are required to be pairwise disjoint.  We shall always assume that any
rational component of $C$ has at least \textbf{two} special points.

We reserve the notation $(\Sigma,\sigma_i)$ for families of \textbf{stable}
...
$1$ carries at least $1$ special point.  The \textbf{stabilization morphism}
...
\textbf{Gieseker $\C^\times$-bundles on stable genus $g$, $I$-marked curves}
...
\begin{enumerate}
\item[\textnormal{(1)}] \textbf{Resolving a self-node.}
...
\item[\textnormal{(2)}] \textbf{Resolving a splitting node.}
...
\textbf{smooth} existing nodes; new nodes do not appear.  Thus the only
...
which are \textbf{not} smoothed in the deformation and look at each
...
residue field~$\C$.  The \textbf{deformation functor} of
...
For the bundle, one has the classical \textbf{Atiyah class} of a principal
...
\textbf{Atiyah bundle} of $\mathcal P$.  Twisting by $-\sum_i\sigma_i$ to fix
...
\]
with $H^0$ in degree $-1$ and $H^1$ in degree~$0$.  The \textbf{virtual
...
\begin{enumerate}
  \item[\textnormal{1.}] \textbf{Self node:} Lower the genus of a vertex by $1$
    and add a self-edge.

  \item[\textnormal{2.}] \textbf{Splitting node:} Split a vertex $v$ into two
    vertices $v_1$ and $v_2$, connected by an edge, with
    $g_{v_1}+g_{v_2}=g_v$ and $d_{v_1}+d_{v_2}=d_v$.

  \item[\textnormal{3.}] \textbf{Gieseker bubbling:} Replace an edge connecting a
...
\]
We will present $\widetilde{\mathcal M}|_B$ as a quotient stack by trivializing the bundles at special points. Recall that for a prestable curve $C$ with a special point $\sigma : B \to C$, a \textbf{trivialization} at $\sigma$ of a principal $\C^\times$-bundle $\mathcal P \to C$ is an isomorphism
\[
...
The \textbf{local chart} $A$ for $\widetilde{\mathcal M}|_B$ is the stack of Gieseker bundles over the curve $\Sigma \to B$, equipped with a trivialization $t_v$ at each $\sigma_v$; isomorphisms are required to be compatible with the trivializations.
\end{definition}
...
A Gieseker bundle $(\mathcal P,C)$ of total degree $G$ \textbf{meets the genus
...
The \textbf{stable subspace} $A_G^\circ\subset A_G$ comprises the bundles
...
study its properties, we first introduce a \textbf{deformation type}
...
We call $c:\gamma_0\to\gamma$ the \textbf{deformation type} (with respect to
$\Sigma_0$) of any curve $\Sigma$ parametrized by $B_c$.  Likewise, the
\textbf{deformation type} of a Gieseker bundle $\mathcal P$ of multi-degree
...
Let $S = \Spec\C[[s]]$, $S^* = \Spec\C((s))$, $S_0 = \Spec\C[[s]]/(s) = \Spec\C$,
...
\begin{enumerate}[(i)]
  ...
  \item for each $v\in V$, a \textbf{trivialization at $\sigma_v$} is an isomorphism
        \[
...
\]
\end{proposition}
We need to establish the following properties of the stack $\mathcal X_{G,g,I}$:
\begin{enumerate}
\item $\mathcal X_{G,g,I}$ is algebraic and locally of finite type over $\overline{\mathfrak M}_{g,I}$.

\item After \'etale localization to $B\to\overline{\mathfrak M}_{g,I}$, we have
\[
...
These properties, combined with the local cohomology argument from \S 6, imply that for any admissible class $\alpha$, the $H$-invariant part of $R\Gamma(A,\alpha)$ is a finitely generated $\mathcal O_B$-module that vanishes in high degrees. Since this holds étale-locally over $B$ and the stack structure descends, we conclude that the derived pushforward
\[
RF_* \alpha : \mathcal X_{G,g,I} \longrightarrow \overline{\mathfrak M}_{g,I}
\]
is a bounded complex of coherent sheaves, where $F$ is the forgetful map forgetting the bundle and the modification.
\subsection{Step 1: Algebraicity and finite type of $\mathcal X_{G,g,I}$}

\begin{proposition}\label{prop:X-algebraic}
The stack $\mathcal X_{G,g,I}$ is an algebraic stack, locally of finite type over
$\overline{\mathfrak M}_{g,I}$. In particular, the projection
\[
F:\mathcal X_{G,g,I}\to \overline{\mathfrak M}_{g,I}
\]
...
\[
\mathfrak M^{tw}_{g,I}\to \overline{\mathfrak M}_{g,I}
\]
sending a twisted curve to its coarse stable curve.

\medskip
\noindent\textbf{Step 2: Twisted modifications.}
Let $\mathsf{TwMdf}_{\le r,g,I}$ be the stack over $\overline{\mathfrak M}_{g,I}$
whose objects over a scheme $B\to \overline{\mathfrak M}_{g,I}$ are twisted
modifications of length $\le r$ of the pulled--back universal curve $\Sigma_B\to B$.

Étale--locally on $\overline{\mathfrak M}_{g,I}$, the universal curve is
simultaneously versal at all nodes. Fix an \'etale chart $B \to \overline{\mathfrak M}_{g,I}$ and pull back the universal curve
\[
\Sigma := \overline{\mathfrak C}_{g,I}\times_{\overline{\mathfrak M}_{g,I}} B \to B.
\]

Let $\Sigma^{\mathrm{sing}} := \Sing(\Sigma/B)\subset \Sigma$. Over the locus in $B$ where the number of nodes is constant, $\Sigma^{\mathrm{sing}}\to B$ is finite \'etale (after possibly refining $B$ \'etale). Concretely this means you can label the nodes locally on $B$, giving disjoint sections
\[
p_1,\dots,p_m: B \to \Sigma
\]
landing in the singular locus.

Around each $p_j$, the family $\Sigma\to B$ has the standard node-smoothing normal form:
\'etale-locally on $\Sigma$ and $B$,
\[
\Sigma \;\sim\; \Spec\bigl(\mathcal O_B[x,y]/(xy-t_j)\bigr)
\]
for some function $t_j \in \Gamma(B,\mathcal O_B)$.
Here $t_j=0$ is exactly the boundary divisor where the $j$-th node persists.

Simultaneously versal at all nodes means after an \'etale refinement of $B$, you can choose these local coordinates for all nodes at once, and the deformation space near your point looks like a normal crossings divisor cut out by $\prod_j t_j=0$.


Twisted modifications of bounded length
are obtained by inserting chains of $\P^1$'s and taking balanced root stacks at
the nodes. Standard results on expanded degenerations and twisted curves imply
that $\mathsf{TwMdf}_{\le r,g,I}$ is an algebraic stack, locally of finite type over
$\overline{\mathfrak M}_{g,I}$.

\begin{proposition}[ACFW: twisted expansions are algebraic and lft]
...
\begin{example}[Twisted expansions of the universal node]
    The fundamental example: a single smoothing node

    Start with the universal local model of a node:
    \[
    U \;=\; \Spec \C[x,y,t]/(xy-t) \;\longrightarrow\; \Spec \C[t].
    \]
    The central fiber $t=0$ has a node. For $t\neq0$ it is smooth.
    Everything about twisted expansions is already visible here.

    \medskip\noindent\textbf{1. Untwisted expansion of length 1}

    Replace the single node by one $\mathbb P^1$.
    Geometrically: you blow up the ideal $(x,t)$ or $(y,t)$. The central fiber becomes:
    \[
    (\text{branch})\;-\;\mathbb P^1\;-\;(\text{branch})
    \]
    Locally this is controlled by parameters $t_0,t_1$ with
    $t_0 t_1 = t$.

    The moduli of such expansions is:
    \[
    \Spec \C[t_0,t_1] \big/ \mathbb{G}_m
    \]
    where $\lambda \in \mathbb{G}_m$ acts by
    $(t_0,t_1) \mapsto (\lambda t_0, \lambda^{-1} t_1)$.

    This is exactly Jun Li's expansion stack.

    \medskip\noindent\textbf{2. Untwisted expansion of length $n$}

    Insert a chain of $n$ rational curves.
    Local equation:
    \[
    t_0 t_1 \cdots t_n = t.
    \]
    Moduli stack:
    \[
    [\Spec \C[t_0,\dots,t_n] / (\mathbb{G}_m)^n].
    \]
    Each $\mathbb{G}_m$ rescales one intermediate component.

    This is the local chart of the stack of expansions.

    \medskip\noindent\textbf{3. Now twist it: add stabilizers at the new nodes}

    Pick an integer $k$.
    Replace each node in the expanded curve by the stacky node:
    \[
    [\Spec \C[u,v]/(uv=t_i) \;/\; \mu_k],
    \qquad
    \zeta\cdot(u,v) = (\zeta u, \zeta^{-1} v).
    \]
    This is a root stack along the divisor $t_i=0$.

    Geometrically:
    \begin{itemize}
    \item coarse space: the expanded curve
    \item stack structure: $\mu_k$ sitting at the nodes
    \end{itemize}

    This is what ``twisted expansion'' means.

    \medskip\noindent\textbf{4. Twisted expansion stack}

    Now let:
    \begin{itemize}
    \item the length vary
    \item the parameters $t_i$ vary
    \item the twisting orders $k$ vary (or be bounded)
    \end{itemize}

    The resulting moduli problem is exactly the ACFS stack $\mathfrak T^{\mathrm{tw}}$.

    Every object of $\mathfrak T^{\mathrm{tw}}$ is built by:
    \[
    \text{node} \;\longrightarrow\;
    \text{chain of }\mathbb{P}^1\text{s} \;\longrightarrow\;
    \text{stacky nodes}.
    \]

    No global curve geometry is involved. It is purely local.
\end{example}



\begin{example}[Admissibility for $G=SL_2$]
\label{ex:SL2-admissible}

Suppose $C$ has a single node and let $\mathcal C'$ be a twisted modification of
$C$ whose fiber over the node is a chain of $m$ rational curves
\[
R_1 \cup R_2 \cup \cdots \cup R_m,
\]
with stabilizer $\mu_k$ at each twisted node. Let $\mathcal P$ be a principal
$SL_2$--bundle on $\mathcal C'$.

At each twisted node, $\mathcal P$ determines (up to conjugacy) a homomorphism
\[
\rho_i : \mu_k \to SL_2(\C).
\]
After choosing a maximal torus, each $\rho_i$ is of the form
\[
\rho_i(\zeta)
=
\begin{pmatrix}
...
\end{pmatrix}
\qquad
\text{for some } a_i \in \Z/k\Z.
\]
The tuple $(a_1,\dots,a_m)$ records the local type of $\mathcal P$ along the
expanded node.

The cocharacter lattice of $SL_2$ is $X_*(T)\cong \Z$, so
$X_*(T)\otimes_\Z \Q \cong \Q$ has dimension one. Solis's admissibility condition
therefore specializes to:
\begin{enumerate}[(i)]
\item $a_i \neq 0$ for all $i$;
\item the set $\{a_1,\dots,a_m\}$ is $\Q$--linearly independent in $\Q$.
\end{enumerate}
Since any two nonzero elements of $\Q$ are $\Q$--linearly dependent, condition
(ii) forces $m\le 1$. Hence an admissible twisted modification for $SL_2$ contains
at most one twisted rational component over each node, and its associated
character $a\in \Z/k\Z$ must be nonzero.

Equivalently, on the unique inserted component $R\simeq \P^1$, the associated
rank--$2$ vector bundle splits as
\[
\mathcal P|_R \;\cong\; \mathcal O_{\P^1}(a)\oplus \mathcal O_{\P^1}(-a)
\qquad\text{with } a\neq 0.
\]
\end{example}
...
curve $\mathcal C\to B$ the groupoid of principal $G$--bundles on $\mathcal C$.
For reductive $G$, this stack is algebraic and locally of finite presentation over
$\mathfrak M^{tw}_{g,I}$ (by standard results on principal bundles on proper
...
finite type over $\overline{\mathfrak M}_{g,I}$.

\medskip
\noindent\textbf{Step 5: Admissibility.}
The admissibility condition is a restriction on the local monodromy homomorphisms
...
\begin{lemma}[Local constancy of monodromy types]
\label{lem:local-constancy-monodromy}

Let $B$ be a scheme and let $\Sigma_B \to B$ be a prestable family of curves.
Let $\mathcal Y$ be the stack over $B$ whose objects are twisted modifications
$\mathcal C'_B \to \Sigma_B$ together with principal $G$--bundles $\mathcal P_B$ on
$\mathcal C'_B$.

Assume that, after possibly replacing $B$ by an \'etale cover, the following hold:
\begin{enumerate}[(i)]
\item The relative singular locus $\Sing(\Sigma_B/B)$ is a disjoint union of sections
\[
p_1,\dots,p_m : B \to \Sigma_B.
\]
\item Over each $p_j$, the stabilizer group of the twisted curve $\mathcal C'_B$ is
constant and equal to $\mu_{k_j}$.
\end{enumerate}

Then for each $j$, the conjugacy class of the local monodromy homomorphism
\[
\rho_j : \mu_{k_j} \longrightarrow G
\]
is locally constant on $B$.

Equivalently, $\mathcal Y|_B$ decomposes as a disjoint union of open-and-closed substacks
indexed by the tuples
\[
([\rho_1],\dots,[\rho_m]) \in \prod_{j=1}^m \Hom(\mu_{k_j},G)/G.
\]

\end{lemma}

\begin{proof}
Fix $j$. Restriction of $(\mathcal C'_B,\mathcal P_B)$ to the residual gerbe of the twisted
point over $p_j$ gives a principal $G$--bundle on $B\mu_{k_j}\times B$. Such a bundle is
equivalent to a morphism
\[
B \longrightarrow \Hom(\mu_{k_j},G)/G.
\]
The target is a finite discrete algebraic space. Hence this morphism is locally constant
on $B$. This proves the claim.
\end{proof}

\begin{corollary}[Admissible locus]
\label{cor:admissible-open-closed}

With notation as above, suppose that for each $j$ we fix a finite subset
\[
\mathcal A_j \subset \Hom(\mu_{k_j},G)/G.
\]
Define $\mathcal X_{G,g,I}|_B \subset \mathcal Y|_B$ to be the full substack of objects
whose local monodromies satisfy $[\rho_j] \in \mathcal A_j$ for all $j$.

Then $\mathcal X_{G,g,I}|_B$ is an open-and-closed substack of $\mathcal Y|_B$.
\end{corollary}

\begin{proof}
By Lemma~\ref{lem:local-constancy-monodromy}, $\mathcal Y|_B$ decomposes into a disjoint
union of open-and-closed substacks indexed by the tuples $([\rho_1],\dots,[\rho_m])$.
Since only finitely many types are allowed, the admissible locus is the union of finitely
many connected components, hence open and closed.
\end{proof}

\begin{lemma}[Local constancy for chains of twisted components]
\label{lem:local-constancy-chains}

Assume the hypotheses of Lemma~\ref{lem:local-constancy-monodromy}. Suppose moreover that
over each section $p_j$, the fiber of $\mathcal C'_B \to \Sigma_B$ is a chain of twisted
rational curves
\[
R_{j,1} \cup \cdots \cup R_{j,\ell_j}
\]
with twisted nodes $q_{j,1},\dots,q_{j,\ell_j}$, and that the stabilizer groups at these
twisted nodes are constant and equal to $\mu_{k_{j,1}},\dots,\mu_{k_{j,\ell_j}}$.

Then for every pair $(j,r)$, the conjugacy class of the local monodromy homomorphism
\[
\rho_{j,r} : \mu_{k_{j,r}} \longrightarrow G
\]
is locally constant on $B$. Consequently, $\mathcal Y|_B$ decomposes as a disjoint union of
open-and-closed substacks indexed by the finite set of tuples
\[
([\rho_{j,r}])_{j,r}.
\]
\end{lemma}
...
\begin{proof}
Each twisted node $q_{j,r}$ defines a closed substack
$B\mu_{k_{j,r}} \times B \subset \mathcal C'_B$. Restriction of $\mathcal P_B$ to this gerbe
defines a morphism
\[
B \to \Hom(\mu_{k_{j,r}},G)/G,
\]
which is locally constant because the target is discrete. The conclusion follows by
taking the product over all twisted nodes.
\end{proof}

\begin{remark}
The key point is that a principal $G$--bundle on a twisted curve carries, at each twisted
node, a discrete invariant: a conjugacy class of homomorphisms $\mu_k \to G$. In a flat
family these invariants cannot vary continuously. Hence the moduli stack splits into
connected components indexed by local monodromy data, and admissibility simply selects
...
$\overline{\mathfrak M}_{g,I}$. This proves the proposition.
\end{proof}

\subsection{Step 2: Etale presentation by an algebraic space}
Fix a geometric point $[\Sigma_0,\sigma_{0,i}] \in \overline{\mathfrak M}_{g,I}$ and let
$B\to \overline{\mathfrak M}_{g,I}$ be an affine \'etale neighborhood. Let
\[
\Sigma := \overline{\mathfrak C}_{g,I}\times_{\overline{\mathfrak M}_{g,I}} B \to B
\]
...
Let $\mathcal X|_B := \mathcal X_{G,g,I}\times_{\overline{\mathfrak M}_{g,I}} B$.
...
\begin{lemma}[The framing map is an $H$--torsor]
\label{lem:framing-torsor}
The morphism $\pi:A\to \mathcal X|_B$ is representable and smooth. For any $T\to\mathcal X|_B$
corresponding to $(\mathcal C'_T\to \Sigma_T,\mathcal P_T)$, the fiber product
$A\times_{\mathcal X|_B} T \to T$ is a principal $H$--bundle for the fppf topology.
\end{lemma}

\begin{proof}
Fix $T\to\mathcal X|_B$ as above. For each $v\in V$, the sheaf
\[
\Isom\bigl((\sigma_{v,T})^*\mathcal P_T,\ G_T\bigr)
\]
is a (right) $G$--torsor on $T$. Taking the product over $v\in V$ gives an $H=G^V$--torsor,
which is functorially identified with $A\times_{\mathcal X|_B}T$.
Since $H$ is smooth affine, torsors are representable and smooth over $T$.
\end{proof}

\begin{lemma}[Trivial stabilizers on the framed chart]
\label{lem:framed-trivial-stabilizers}
After possibly shrinking $B$ \'etale-locally and restricting to the locus where the
admissible bundles have automorphism group equal to the center $Z(G)$ on each stable
component, the stack $A$ has trivial stabilizers. Hence $A$ is an algebraic space.
\end{lemma}

\begin{proof}
Let $(\mathcal C'_T,\mathcal P_T,(t_v))\in A(T)$ and let $\varphi$ be an automorphism.
By definition $\varphi$ is an automorphism of $\mathcal P_T$ commuting with the framings,
so it restricts to the identity on each framed fiber $(\sigma_{v,T})^*\mathcal P_T$.

On the chosen open locus, any automorphism of $\mathcal P_T$ over a stable component is given
by multiplication by a (necessarily constant) element of $Z(G)$. The framing at one point on
that component forces this element to be $1$. Since every stable component meets some
$\sigma_v$, the automorphism is trivial everywhere. Thus $\varphi=\mathrm{id}$, so stabilizers
are trivial and $A$ is an algebraic space.
\end{proof}

\red{Here we need an analog of the FTT statement “\(\Aut(\mathcal P)\) is computed by deleting the Gieseker bubbles.”}
...
\begin{lemma}[Central automorphisms are constant]
Let $P \to X$ be a principal $G$--bundle over a connected scheme $X$.  
Any automorphism $\Phi$ of $P$ over $X$ is uniquely determined by a section
$g$ of the adjoint bundle $\Ad(P) = P \times^G G$ via
\[
\Phi(p) = p \cdot g(p).
\]
If $\Phi$ is \textbf{central}, i.e.\ if $g(p)\in Z(G)$ for all $p\in P$, then
$g$ is constant along the fibers of $P$ and therefore descends to a morphism
\[
z : X \longrightarrow Z(G).
\]
If moreover $X$ is connected and $G$ is semisimple (so that $Z(G)$ is finite),
then $z$ is constant, hence there exists a fixed $z\in Z(G)$ such that
\[
\Phi(p) = p \cdot z \qquad \text{for all } p\in P.
\]
\end{lemma}

\begin{proof}
For any automorphism $\Phi$, $G$--equivariance implies
$\Phi(p) = p \cdot g(p)$ for a unique map $g:P\to G$ satisfying
$g(p\cdot h) = h^{-1} g(p) h$, hence defining a section of $\Ad(P)$.
If $g(p)\in Z(G)$, then $h^{-1} g(p) h = g(p)$ for all $h\in G$, so $g$ is
constant along $G$--orbits and descends to a morphism $z:X\to Z(G)$.
If $X$ is connected and $Z(G)$ is finite, any such morphism is constant.
\end{proof}


\begin{proposition}[Algebraic-space chart]
The stack $A$ is represented by an algebraic space.
\end{proposition}

\begin{proof}
It suffices to show that geometric points of $A$ have trivial automorphism groups.
Let $(C\to\Sigma,\mathcal P,(t_v))$ be a geometric point.

On the open locus where admissible bundles on stable components have only central
automorphisms (i.e.\ $\Aut(\mathcal P|_{C_v})\subseteq Z(G)$ for each stable component
$C_v$), any automorphism of $\mathcal P$ restricts on $C_v$ to multiplication by a central
element of $G$. Compatibility with the framing at $\sigma_v$ forces this element to be $1$.
Since every stable component contains some $\sigma_v$, the automorphism is trivial
globally. Hence $A$ has trivial stabilizers and is an algebraic space.
\end{proof}


\begin{proposition}[Quotient presentation]
\label{prop:quotient-presentation}
After possibly refining $B$ \'etale-locally (and restricting to the open locus of
Lemma~\ref{lem:framed-trivial-stabilizers}), the natural map induces an equivalence of
stacks over $B$:
\[
\mathcal X_{G,g,I}|_B \;\simeq\; [A/H],
\]
where $A$ is a smooth algebraic space over $B$ and $H=G^V$ is reductive.
\end{proposition}

\begin{proof}
By Lemma~\ref{lem:framing-torsor}, $\pi:A\to\mathcal X|_B$ is a principal $H$--bundle and
$H$ acts simply transitively along fibers by changing framings. Descent along the torsor
$\pi$ identifies the quotient stack $[A/H]$ with $\mathcal X|_B$.
Reductivity of $H$ follows from reductivity of $G$, and smoothness of $A\to B$ follows from
smoothness of $\pi$ together with algebraicity of $\mathcal X|_B\to B$.
\end{proof}
