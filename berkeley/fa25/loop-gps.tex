...
\begin{remark}
    If $\mathfrak{g}$ is simple, then every invariant symmetric bilinear form on $\mathfrak{g}$ is a multiple of the Killing form. So in that case, the cocycle
    \[
        \omega(\xi,\eta) = \frac{1}{2\pi}\int_0^{2\pi} \langle \xi(\theta), \eta'(\theta)\rangle \, d\theta
    \]
    is unique up to scalar.

    But in general, if $\mf g$ is only semisimple, the space of invariant symmetric bilinear forms on $\mathfrak{g}$ will have dimension equal to the number of simple summands. So instead of fixing one $\langle\ ,\ \rangle$, introduce the universal bilinear form:
    \[
        \langle\ ,\ \rangle_K : \mathfrak{g} \times \mathfrak{g} \;\to\; K,
    \]
...
\[
\nabla(a \cdot m) = d(a) \cdot m + (-1)^{|a|} a \cdot \nabla(m)
\]
...
\begin{remark}
We compute $\nabla^2$ on $am$ for $a \in A$ and $m \in M$:
For the first summand we have
\[
\nabla(d(a)m)=d^2(a)m+(-1)^{|d(a)|}d(a)\,\nabla(m)
= d^2(a)m+(-1)^{|a|+1}d(a)\,\nabla(m).
\]
...
\nabla(a\,\nabla(m))=d(a)\,\nabla(m)+(-1)^{|a|}a\,\nabla^2(m),
\]
and multiplying by the prefactor $(-1)^{|a|}$ yields
\[
(-1)^{|a|}\nabla(a\,\nabla(m)) = (-1)^{|a|}d(a)\,\nabla(m) + a\,\nabla^2(m).
\]
...
\[
\nabla^2(am)
= d^2(a)m +\big((-1)^{|a|+1}+(-1)^{|a|}\big)d(a)\,\nabla(m) + a\,\nabla^2(m)
= d^2(a)m + a\,\nabla^2(m),
\]
since $(-1)^{|a|+1}+(-1)^{|a|}=0$. We conclude that
\begin{align*}
\nabla^2(a \cdot m) &= d^2(a) \cdot m + a \cdot \nabla^2(m) \\
&= [W,a] \cdot m + a \cdot W \cdot m \\
&= W \cdot a \cdot m
\end{align*}
so in fact the curvature condition $\nabla^2 = W$ follows from the Leibniz rule.

Note that if you tried to make $A$ into a curved module 
over itself, the algebra differential cannot serve as the module differential on $A$ itself.
\end{remark}
...
and $\iota_a$ the contraction operator. Set
\[
B := \Omega^*(X)\otimes \Sym(\frak g^*).
\]
The $G$-action on $X$ and the coadjoint action on $\frak g^*$ induce an
action of $G$ on $B$ by
\[
g\cdot(\omega\otimes p)
:= (g^{-1})^*\omega\;\otimes\;(g\cdot p).
\]
Let $\mathcal D(G)$ be the convolution algebra of compactly supported distributions on $G$, with unit $\delta_1$ (Dirac delta at the identity). We form the crossed product algebra
\[
\mathcal C := \mathcal D(G)\ltimes B.
\]
As a \emph{vector space} we have
\[
\mathcal C \cong \mathcal D(G)\otimes B,
\]
but the multiplication is twisted by the $G$--action on $B$. We embed $\mathcal D(G)$ and $B$ into $\mathcal C$ via
\[
\iota_G : \mathcal D(G)\longrightarrow \mathcal C,
\qquad
\mu \longmapsto \mu\otimes 1_B,
\]
\[
\iota_B : B\longrightarrow \mathcal C,
\qquad
b \longmapsto \delta_1\otimes b.
\]
Thus an element $\mu\in\mathcal D(G)$ is viewed in $\mathcal C$ as
$\mu\otimes 1_B$, and an element $b\in B$ is viewed as $\delta_1\otimes b$.

The $G$--action on $B$ is
\[
G\times B\to B,\qquad
g\cdot(\omega\otimes p)
:= (g^{-1})^*\omega\;\otimes\;(g\cdot p),
\]
where $g\cdot p$ is the coadjoint action on $\Sym(\mathfrak g^*)$. The crossed product $\mathcal C$ is generated by the images of
$\mathcal D(G)$ and $B$ under $\iota_G,\iota_B$, subject to
\begin{equation}\label{eq:cross-rel-explicit}
\iota_G(\delta_g)\,\iota_B(b)
=
\iota_B(g\cdot b)\,\iota_G(\delta_g)
\qquad
\text{for all } g\in G,\ b\in B.
\end{equation}
Concretely, since
\[
\iota_G(\delta_g) = \delta_g\otimes 1_B,
\qquad
\iota_B(b) = \delta_1\otimes b,
\]
\eqref{eq:cross-rel-explicit} is the identity
\[
(\delta_g\otimes 1_B)\cdot(\delta_1\otimes b)
=
(\delta_1\otimes (g\cdot b))\cdot(\delta_g\otimes 1_B)
\qquad\text{in }\mathcal C.
\]

with differential
\begin{align*}
    d(\delta \otimes (\omega \otimes p)) &= \delta \otimes d(\omega \otimes p) \\
    d &= d_X + \iota_{\xi_a} \otimes \xi^a
\end{align*}
where $d_X:\Omega^*(X) \to \Omega^{*+1}(X)$ is the de Rham differential, $\iota_{\xi_a}:\Omega^*(X) \to \Omega^{*-1}(X)$ is the contraction operator with the vector field on $X$ generated by $\xi_a$, and $\xi^a \in \Sym^1(\mathfrak g^*)$ acts by multiplication. Note that 
the generators $\xi^a$ have degree 2 so $\deg(\iota_{\xi_a}\otimes \xi^a) = (-1) + 2 = 1$
and hence $d$ is homogeneous of degree $1$ with respect to the total Cartan grading.
\end{definition}


Using the Cartan identities
\[d_X^2 = 0,\qquad
[d_X,\iota_{\xi_a}] = L_{\xi_a},\qquad
[\iota_{\xi_a},\iota_{\xi_b}] = 0\]
one checks:
\[
d^2
= d_X^2 + [d_X,\iota_{\xi_a}]\otimes\xi^a
= L_{\xi_a}\otimes \xi^a.
\]
Therefore, applying $d^2$ to an element $[\delta_g] \otimes (\omega \otimes p)$ gives
\begin{equation}
    d^2([\delta_g] \otimes (\omega \otimes p))
= [\delta_g] \otimes (L_{\xi_a}\omega \otimes \xi^a p)
\label{eq:d2-on-B}
\end{equation}
...
\begin{proposition}
    The curved Cartan complex has curvature
\[
W \;=\; \xi_a(\delta_1)\otimes 1 \otimes \xi^a.
\]
where $\delta_1$ is the delta distribution at the identity element of $G$, and $\xi_a$ acts on distributions as follows.
\[
\big\langle X\mu,\varphi\big\rangle \;:=\; -\big\langle \mu,\, X\varphi\big\rangle,
\qquad \varphi\in C^\infty(G),
\]
where $\mu$ is a distribution on $G$, $X$ is a vector field, and $\langle\cdot,\cdot\rangle$ denotes the distribution-test function pairing.
\end{proposition}



\begin{lemma}
Let $\xi_a\in\frak g$ and $\gamma_a(t):=\exp(t\xi_a)$ the corresponding
one-parameter subgroup.  Denote by the same symbol $\xi_a$ the
left-invariant vector field on $G$, and let $\xi_a(\delta_1)$ be its
action on the delta distribution at $1\in G$, i.e.
\[
\langle \xi_a(\delta_1),\varphi\rangle
:= -\langle \delta_1, \xi_a\varphi\rangle,
\qquad \varphi\in C^\infty(G).
\]
Let
\[
\rho(\xi_a)b := \left.\frac{d}{dt}\right|_{t=0} \bigl(\gamma_a(t)\cdot b\bigr)
\qquad (b\in B)
\]
be the infinitesimal action of $\xi_a$ on $B$.  Then in $\cC$ we have
\begin{equation}\label{eq:xi-delta1-comm}
[\xi_a(\delta_1) \otimes 1, \delta_1 \otimes b] = \delta_1 \otimes \rho(\xi_a)b,
\qquad\text{for all }b\in B,
\end{equation}
and hence for all $g\in G$ and $b\in B$,
\begin{equation}\label{eq:xi-delta1-comm-g}
[\xi_a(\delta_1) \otimes 1, \delta_g \otimes b] = \delta_g \otimes \rho(\xi_a)b.
\end{equation}
\end{lemma}

\begin{proof}

Apply \eqref{eq:cross-rel-explicit} with $g=\gamma_a(t)$:
\begin{equation}\label{eq:rel-gamma-t}
\iota_G(\delta_{\gamma_a(t)})\,\iota_B(b)
=
\iota_B(\gamma_a(t)\cdot b)\,\iota_G(\delta_{\gamma_a(t)})
\qquad\text{in }\mathcal C,\ \forall t.
\end{equation}

Differentiate \eqref{eq:rel-gamma-t} at $t=0$ inside $\mathcal C$.
Using the Leibniz rule:

\[
\left.\frac{d}{dt}\right|_{t=0}
\bigl(\iota_G(\delta_{\gamma_a(t)})\,\iota_B(b)\bigr)
=
\left.\frac{d}{dt}\right|_{t=0}
\bigl(\iota_G(\delta_{\gamma_a(t)})\bigr)\,\iota_B(b)
=
\iota_G\bigl(\xi_a(\delta_1)\bigr)\,\iota_B(b),
\]
\[
\begin{aligned}
\left.\frac{d}{dt}\right|_{t=0}
\bigl(\iota_B(\gamma_a(t)\cdot b)\,\iota_G(\delta_{\gamma_a(t)})\bigr)
&=
\left.\frac{d}{dt}\right|_{t=0}\iota_B(\gamma_a(t)\cdot b)\,\iota_G(\delta_1)
\;+\;
\iota_B(b)\,\left.\frac{d}{dt}\right|_{t=0}\iota_G(\delta_{\gamma_a(t)})\\[4pt]
&=
\iota_B\bigl(\rho(\xi_a)b\bigr)\,\iota_G(\delta_1)
\;+\;
\iota_B(b)\,\iota_G\bigl(\xi_a(\delta_1)\bigr).
\end{aligned}
\]

Since $\iota_G(\delta_1)$ is the unit of $\mathcal C$, we have
$\iota_B(\rho(\xi_a)b)\,\iota_G(\delta_1)=\iota_B(\rho(\xi_a)b)$.  Thus
the derivative of \eqref{eq:rel-gamma-t} at $t=0$ gives the equality
in $\mathcal C$:
\begin{equation}\label{eq:xi-delta1-b-full}
\iota_G\bigl(\xi_a(\delta_1)\bigr)\,\iota_B(b)
=
\iota_B\bigl(\rho(\xi_a)b\bigr)
\;+\;
\iota_B(b)\,\iota_G\bigl(\xi_a(\delta_1)\bigr).
\end{equation}

Rearranging \eqref{eq:xi-delta1-b-full}, we obtain the commutator identity
\begin{equation}\label{eq:xi-delta1-comm-full}
\bigl[\iota_G(\xi_a(\delta_1)),\,\iota_B(b)\bigr]
:=
\iota_G(\xi_a(\delta_1))\,\iota_B(b)
-
\iota_B(b)\,\iota_G(\xi_a(\delta_1))
=
\iota_B\bigl(\rho(\xi_a)b\bigr)
\end{equation}
in the crossed product algebra $\mathcal C$.

Explicitly, in the tensor notation $\mathcal C\cong\mathcal D(G)\otimes B$,
this reads
\[
\bigl[(\xi_a(\delta_1)\otimes 1_B),\,(\delta_1\otimes b)\bigr]
=
\delta_1\otimes \rho(\xi_a)b. \qedhere
\] 
\end{proof}
...
\begin{proposition}
For all $a\in\cC$ we have
\[
d^2(a) = [W,a].
\]
where 
\[
W = \xi_a(\delta_1)\otimes 1\otimes \xi^a \;\in\; \cC^2.
\]
\end{proposition}

\begin{proof}
Since elements of the form $\delta_g\otimes b$ span $\cC$, it suffices
to check the identity on such generators.  Using that $\xi^a$ is central
in $B$, we compute
\[
\begin{aligned}
[W,\,\delta_g\otimes b]
&= [\,\xi_a(\delta_1)\otimes 1\otimes\xi^a,\;\delta_g\otimes b\,] \\
&= \bigl([\xi_a(\delta_1),\,\delta_g b]\bigr)\otimes\xi^a \\
&\overset{\eqref{eq:xi-delta1-comm-g}}=
   \delta_g\otimes\bigl(\rho(\xi_a)b\bigr)\otimes\xi^a.
\end{aligned}
\]
For $b\in B$, the infinitesimal action $\rho(\xi_a)$ decomposes into the
Lie derivative on the $\Omega^*(X)$-factor and the coadjoint action on
the $\Sym(\frak g^*)$-factor.  In particular, using \eqref{eq:d2-on-B},
we have
\[
d^2(b) = L_{\xi_a}b\otimes\xi^a = \rho(\xi_a)b\otimes\xi^a,
\]
with the chosen Cartan differential.  Therefore
\[
[W,\,\delta_g\otimes b]
= \delta_g\otimes d^2(b)
= d^2(\delta_g\otimes b),
\]
which proves $d^2 = [W,-]$ on $\cC$.
\end{proof}

The meaning of the curved Cartan complex is best understood in terms of group actions on categories. Its category of curved modules corresponds to the $G/\hat{G}$-fixed category of $(\Omega^*(X), d_X)$-modules, where $\hat{G} \hookrightarrow G$ is the formal completion of $G$ at the identity.
