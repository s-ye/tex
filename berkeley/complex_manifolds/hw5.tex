...
\begin{remark}[1]
  A unitary character is a homomorphism $\chi: L \longrightarrow U(1)$.

  Any $\chi$ can be written as $\chi(\ell)=\exp(2\pi i\,\xi(\ell))$ for some real-valued group homomorphism $\xi: L\to\R$. Conversely, any such $\xi$ defines a character this way. So
  \[
  ...
  But
  \[
  \Hom(L,\R) \cong L^\vee\otimes_{\Z}\R \cong \overline{V}^\vee
  \]
  (using the inclusion $L\subset V$ and identifying $V \cong L\otimes_\Z\R$), and
  \[
  \Hom(L,\Z)\cong L^\vee.
  \]
...
Now suppose $X$ is compact K\"ahler.  Consider the \emph{constant exponential
sequence}
\[
...
\]
and the \emph{holomorphic exponential sequence}
\[
...
give a morphism of short exact sequences and therefore a natural map
\[
H^1(X;U(1))\longrightarrow H^1(X;\cO^\times)\cong\Pic(X).
\]

The connecting homomorphism in the holomorphic sequence
\[
c_1^{\mathrm{hol}}\colon H^1(X;\cO^\times)\longrightarrow H^2(X;\Z)
\]
is the first Chern class of the corresponding holomorphic line bundle; let
\[
\Pic^0(X):=\ker\bigl(c_1^{\mathrm{hol}}\bigr)
\]
be the subgroup of holomorphic line bundles with $c_1=0$.

The long exact sequence of the constant exponential sequence yields
\[
H^1(X;\R)\longrightarrow H^1(X;U(1))
\longrightarrow H^2(X;\Z)\longrightarrow H^2(X;\R),
\]
so $H^1(X;U(1))$ is an extension of $H^1(X;\R)/H^1(X;\Z)$ by the torsion
subgroup of $H^2(X;\Z)$.  Similarly, the long exact sequence of the
holomorphic exponential sequence gives
\[
H^1(X;\cO)\longrightarrow H^1(X;\cO^\times)
\xrightarrow{\,c_1^{\mathrm{hol}}\,} H^2(X;\Z)\longrightarrow H^2(X;\cO),
\]
and exactness shows
\[
\Pic^0(X) \;\cong\; H^1(X;\cO)/H^1(X;\Z).
\]

By Hodge decomposition, on a compact K\"ahler manifold
\[
H^1(X;\C)\cong H^{1,0}(X)\oplus H^{0,1}(X),
\]
and $H^1(X;\cO)\cong H^{0,1}(X)$.  The inclusion $\R\hookrightarrow\cO$
induces a map
\[
H^1(X;\R)\longrightarrow H^1(X;\cO)\cong H^{0,1}(X)
\]
which, under Hodge decomposition, is the projection
$H^{1,0}(X)\oplus H^{0,1}(X)\to H^{0,1}(X)$. 

Passing to the quotients by
$H^1(X;\Z)$, we obtain an isomorphism of real tori
\[
\frac{H^1(X;\R)}{H^1(X;\Z)}\;\xrightarrow{\ \sim\ }\;
\frac{H^1(X;\cO)}{H^1(X;\Z)}\;\cong\;\Pic^0(X).
\]

Thus we see that the image of
$H^1(X;U(1))$ is precisely $\Pic^0(X)$ and that the induced map
\[
H^1(X;U(1)) \;\xrightarrow{\ \sim\ }\; \Pic^0(X)
\]
is a bijection. 
\end{solution} 
