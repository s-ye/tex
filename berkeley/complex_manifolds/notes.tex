...
$\bar\partial_L(f s) = (\bar\partial f)\otimes s \;+\; f\,\bar\partial_L s,
...
Therefore, the map \begin{align*}
    H^1(X, \R) & \to H^{0,1}(X) \\
    [\alpha] & \mapsto [\alpha^{0,1}]
\end{align*} is an isomorphism on $\R$-vector spaces.
\end{remark}
...
positive $(1,1)$-form. Therefore, any ample line bundle $L$ over a compact
