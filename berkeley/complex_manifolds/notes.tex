...
and the resulting map to $J_d$ is now also independent of your choice of $r$.
...
\]
    ...
    In general, there's a uniform way to describe the differential valid for all $D$:
...
\begin{theorem}
    $\Theta \cong \Lambda^{\mathrm{top}} \cH_1 \otimes (\Lambda^{\mathrm{top}} \cH_0)^{-1}$ where $\Theta$ is the line bundle on $J_{g-1}$ with divisor $W_{g-1}$. The map $\mathrm{pp}:\cH_0 \to \cH_1$ induces a holomorphic section of $\Theta$ whose zero locus is precisely $W_{g-1}$.
\end{theorem}
