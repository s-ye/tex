...
$\deg(\omega) = 2g-2 = -\chi(X)$.

In particular, there is the isomorphism of line bundles
$K_R = f^*K_S + B$.
...
For a meromorphic function on $R$, define its \textbf{trace} $\mathrm{Tr}_\pi a$ along $\pi$ 
...
\item By Riemann-Roch, for $d \geq 2g - 1$, we have \begin{align*}
    h^0(R, L) & = d - g + 1
\end{align*} for all line bundles $L$ of degree $d$ because $H^1(R, L) = 0$ by Serre duality. Therefore, the fibers of $AJ_d$ are all projective spaces of dimension $d - g$. This gives $\Sym^d(R)$ the structure of a holomorphic projective bundle over $J_d$. \qedhere
\end{enumerate}
\end{proof}
...
\section{Kodaira Nakano vanishing}
