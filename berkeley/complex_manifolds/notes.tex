...
\]
Then $\phi$ is called the \emph{local weight} of the metric $h$ in the frame
...
= A\times A^\vee
\]
is therefore a holomorphic line bundle, the \emph{Poincar\'e bundle}.  The
...
Let $TF(H^1(X;U(1)))$ denote the torsion-free part of $H^1(X;U(1))$. Using the fact that the kernel of $H^2(X;\Z)\to H^2(X;\R)$ is the torsion subgroup of $H^2(X;\Z)$, we see that the above diagram induces the following commutative diagram:
\begin{align*}
\begin{array}{ccccccccc}
  0 &\to H^1(X;\R)/H^1(X;\Z) &\to TF(H^1(X;U(1))) &\to 0
  \\  
  &\downarrow &\downarrow &
  \\
  0 &\to H^1(X;\cO)/H^1(X;\Z) &\to \Pic^0(X) &\to 0
\end{array}
\end{align*}
in which all the maps are isomorphisms.  The left vertical map is induced by the inclusion $\R\hookrightarrow\cO$ and is an isomorphism by the Hodge decomposition theorem, which gives
\[H^1(X;\R) \cong H^{0,1}(X)\] sending a harmonic representative to its $(0,1)$-part. The horizontal maps are isomorphisms by the exact sequences.  Thus the middle vertical map is an isomorphism
\[TF(H^1(X;U(1))) \longrightarrow \Pic^0(X)\] and thus every flat unitary line bundle with non torsion Chern class corresponds precisely to a holomorphic line bundle with zero Chern class.
\end{solution} 
