...
\begin{abstract}
    These notes accompany Math 241 (Complex Manifolds), taught by Professor Constantin Teleman, Fall 2025, UC Berkeley.  They present a self-contained treatment of the elementary theory of Riemann surfaces functions and differentials, line bundles, Riemann-Roch, Abel-Jacobi maps, and theta functions. This is followed by an introduction to higher-dimensional complex manifolds: holomorphic vector bundles, sheaves, and Dolbeault cohomology.  The exposition culminates in the central results for Kähler geometry, including Hodge decomposition, the Hard Lefschetz theorem, Serre duality for vector bundles, and Kodaira vanishing and embedding theorems.
\end{abstract}

\tableofcontents
\section{Riemann Surfaces}
We introduce Riemann surfaces as analytic subsets of $\C^2$ defined by holomorphic equations. We then develop the structure theory of Riemann surfaces, including holomorphic maps, ramification, and the Riemann-Hurwitz formula. We present the Weierstrass Division and Preparation Theorems, which are fundamental results about the local structure of analytic sets in several complex variables. We examine closely the theory of theta functions on complex tori. We establish the correspondence between Riemann surfaces and finite degree field extensions of $\C(z)$.
\subsection{Structure theory}
\begin{definition}[Holomorphic function in two variables]
A complex function $F(z,w)$ defined in an open set in $\C^2$ is called
\textbf{holomorphic} if, near each point $(z_0,w_0)$ in its domain, $F$
admits a convergent power series expansion
\[
  F(z,w) = \sum_{m,n\ge 0} F_{mn} (z - z_0)^m (w - w_0)^n .
\]
\end{definition}

\begin{definition}[Analytic Riemann surface in $\C^2$]
A subset $S\subseteq \C^2$ is called a \textbf{(concrete, possibly singular)
Riemann surface} if, for each point $s\in S$, there exists a neighbourhood
$U$ of $s$ and a holomorphic function $F$ on $U$ such that $S\cap U$ is the
zero-set of $F$ in $U$. Moreover, we require that
\[
  \frac{\partial^n F}{\partial w^n}(s) \neq 0
\quad\text{for some } n.
\]

In particular, the continuity of $F$ implies that $S$ is locally closed.
The condition $\partial^n F/\partial w^n (s)\neq 0$ rules out vertical
lines through~$s$, which cannot reasonably be viewed as graphs.
(Indeed, from the power series expansion we see that $S\cap U$ contains
a vertical line precisely when $F_{0n}=0$ for all $n$.)
\end{definition}

\begin{definition}[Non-singular point]
The Riemann surface $S$ is called \textbf{non-singular} at $s\in S$ if a
function $F$ defining $S$ near $s$ can be found with gradient vector
$(\partial F/\partial z,\, \partial F/\partial w)$ nonzero at $s$.
\end{definition}

\begin{definition}
    An \textbf{abstract Riemann surface} is a topological surface $R$ equipped with a maximal atlas of charts to open subsets of $\C$ such that the transition functions are holomorphic.
\end{definition}

Every non-singular concrete Riemann surface is an abstract Riemann surface in a natural way, by using the implicit function theorem to produce local charts. However, the converse fails for the obvious reason that compact Riemann surfaces cannot be embedded in $\C^2$ as concrete Riemann surfaces. 

\begin{definition}
    Let $R$ and $S$ be Riemann surfaces. A map $f : R \to S$ is called \textbf{holomorphic} if for every holomorphic function $g : U \to \C$ defined on an open subset $U \subseteq S$, the composition $g \circ f : f^{-1}(U) \to \C$ is a holomorphic function on $R$.
\end{definition}
...
\begin{proof}
Choose local coordinates $z$ near $p$ and $w$ near $q=f(p)$ with
$z(p)=0$, $w(q)=0$, and consider the resulting holomorphic map
\[
F := w \circ f \circ z^{-1} : (\C,0)\longrightarrow(\C,0).
\]
Then $F$ is holomorphic, $F(0)=0$, and $F$ is not constant.

Write the Taylor expansion of $F$ at $0$:
\[
F(z) = \sum_{n=0}^\infty a_n z^n.
\]
Since $F(0)=0$ and $F$ is not constant, there is a smallest integer
$k \ge 1$ such that $a_k \neq 0$. Thus we can factor
\[
F(z) = z^k u(z),
\]
where
\[
u(z) := a_k + a_{k+1}z + a_{k+2}z^2 + \cdots
\]
is holomorphic and satisfies $u(0)=a_k\neq 0$.

Shrink the coordinate disc so that $u(z)$ is never zero there. On a
simply connected neighbourhood of $0$ avoiding the zeros of $u$, a
nowhere vanishing holomorphic function admits a holomorphic logarithm:
there exists a holomorphic function $h$ such that
\[
u(z) = e^{h(z)}.
\]
Define a holomorphic $k$-th root of $u$ by
\[
v(z) := u(z)^{1/k} := e^{h(z)/k}.
\]
Then $v$ is holomorphic, $v(0) \neq 0$, and $v(z)^k = u(z)$.

Now define a new local coordinate on the source by
\[
\zeta := z\,v(z).
\]
Since $v$ is holomorphic and $v(0)\neq 0$, we have
\[
\frac{d\zeta}{dz}\Big|_{z=0} = v(0) \neq 0,
\]
so by the holomorphic inverse function theorem, $\zeta$ is a valid
holomorphic coordinate near $z=0$.

In terms of this new coordinate, we compute
\[
F(z) = z^k u(z) = z^k v(z)^k = (z v(z))^k = \zeta^k.
\]
Thus, in the coordinates $\zeta$ on $R$ and $w$ on $S$, the map $f$ is
given by
\[
w = \zeta^k
\]
near $p$. This proves the existence of the local normal form.

Uniqueness of $k$ follows from the uniqueness of the order of vanishing:
if $F(z) = z^k u(z)$ with $u(0)\neq 0$, then $k$ is exactly the order of
the zero of $F$ at $0$, which is independent of the choice of local
coordinates. Hence $k$ is well defined and is called the ramification
index of $f$ at $p$.
\end{proof}

\begin{corollary}
Let $f : R \to S$ be a nonconstant holomorphic map between Riemann surfaces.
Then $f$ is an open map: the image of every open set in $R$ is open in $S$.
\end{corollary}
\begin{proof}
Let $U \subseteq R$ be an open set and let $q \in f(U)$. Choose $p \in U$ with $f(p)=q$. By the local normal form theorem, there exist local coordinates $z$ near $p$ and $w$ near $q$ such that $w \circ f \circ z^{-1}(\zeta) = \zeta^k$ for some $k \ge 1$. Since $z(U)$ is open in $\C$, it contains a small disc around $0$. The image of this disc under $\zeta \mapsto \zeta^k$ is also a small disc around $0$, which shows that $f(U)$ contains a neighbourhood of $q$. Thus $f(U)$ is open in $S$.
\end{proof}
...
\begin{proof}
Let $f : R \to \C$ be a holomorphic function on a compact connected Riemann surface $R$. Since $R$ is compact, the image $f(R)$ is a compact subset of $\C$. The only compact open subsets of $\C$ are the empty set and $\C$ itself. Since $f(R)$ is nonempty, it must be all of $\C$. However, $\C$ is not compact, so this is a contradiction unless $f(R)$ is a single point. Thus $f$ is constant.
\end{proof}
...
The suprising fact about holomorphic functions in several variables is that they behave like polynomials in many respects. The following theorems establish that analytic sets have algebraic-like local structure and are important in estbalishing general factorization, dimension, flatness, and coherence theorems.
...
\begin{theorem}[Weierstrass Preparation Theorem]
    Let $F$ be holomorphic in the variables $z = (z_1,\dots,z_k)$ and $w$ near the origin in $\C^{k+1}$ and let $n$ be the smallest integer such that
\[
\frac{\partial^{n}F}{\partial w^{\,n}}(0,0)\neq 0.
\]
Then:

\begin{itemize}
\item[\textnormal{(weak form)}]
There exists a function $\Phi$ of the form
\[
\Phi(z,w)
 = w^{n} + f_{n-1}(z) w^{n-1} + \cdots + f_1(z) w + f_0(z),
\]
with $f_0,\dots,f_{n-1}$ analytic near $z=0$, such that the zero--set of
$\Phi(z,w)$ agrees with that of $F(z,w)$ near $(0,0)$.

\item[\textnormal{(strong form)}]
There exists, in addition, a holomorphic function $u(z,w)$, nonzero near
$(0,0)$, such that
\[
F(z,w) \;=\; \Phi(z,w)\,u(z,w).
\]
...
\begin{example}\label{germs-noetherian}
    The ring of germs of holomorphic functions on a complex manifold $X$ can be identified with the ring of convergent power series in $\dim X$ local coordinates. 
The Weierstrass theorems show that the \emph{local} analytic rings
\[
\cO_{X,p} \;\cong\; \C\{z_1,\dots,z_n\}
\]
are Noetherian and factorial (UFD). By contrast, the ring of
\emph{global} holomorphic functions on a domain is typically very far
from either property.

\begin{enumerate}
\item
For example, let $\cO(\C)$ denote the ring of entire functions. For
each $m\ge 1$ define the ideal
\[
I_m := \{\, f\in\cO(\C) : f(1/k)=0 \text{ for all } 1\le k\le m \,\}.
\]
Then $I_m$ consists of entire functions vanishing at the finite set
$\{1,\dots,1/m\}$, hence
\[
I_m = \Bigl(\prod_{k=1}^m (z-1/k)\Bigr),
\]
so each $I_m$ is principal. Clearly
\[
I_1 \;\subsetneq\; I_2 \;\subsetneq\; I_3 \;\subsetneq\; \cdots
\]
is a strictly ascending chain of ideals (each new condition $f(1/(m+1))=0$
cuts out a strictly smaller ideal), and the chain never stabilizes.
Thus $\cO(\C)$ is not Noetherian. Similar chains can be constructed in
$\cO(U)$ for many noncompact domains $U\subset\C^n$.

\item
Even more dramatically, the ring $\cO(\C)$ is not a UFD as exhibited by the infinite product expansion of the sine function:
\[
\sin(\pi z)
...
A_n := \C\{z_1,\dots,z_n\}
\]
is Noetherian for every $n\ge 1$.
\end{theorem}

\begin{proof}
For $n=1$, the ring $\C\{z\}$ is a discrete valuation ring: every
$f\neq 0$ can be written uniquely as $f(z)=z^k u(z)$ with $u(0)\neq 0$,
so every ideal is generated by a single power of $z$, hence $\C\{z\}$ is
Noetherian.

Assume inductively that $A_{n-1}$ is Noetherian, and write
\[
A_n = A_{n-1}\{w\}
\]
with $w=z_n$. Let $I\subset A_n$ be an arbitrary ideal. Choose
$f\in I$ such that its order in $w$ is minimal among all elements of $I$.
By the Weierstrass Preparation Theorem, after multiplying by a unit we may
assume that $f$ is a monic Weierstrass polynomial
\[
P(w) = w^d + a_{d-1}(z') w^{d-1} + \cdots + a_0(z'),
\qquad a_i(0)=0,
\]
lying in $A_{n-1}[w]\subset A_n$.

For any $g\in I$, the Weierstrass Division Theorem gives a unique
expression
\[
g = qP + r,
\]
where $q\in A_n$ and the remainder has the form
\[
r(z',w) = r_0(z') + r_1(z')w + \cdots + r_{d-1}(z')w^{d-1},
\qquad r_i \in A_{n-1}.
\]
Thus
\[
I = (P) + I',
\]
where $I'$ consists of those elements of $I$ of $w$--degree $<d$.
Define a map
\[
\Phi : I' \longrightarrow A_{n-1}^d,\qquad
r \longmapsto (r_0,\dots,r_{d-1}).
\]
The image $J:=\Phi(I')$ is a submodule of the finitely generated
$A_{n-1}$--module $A_{n-1}^d$. Since $A_{n-1}$ is Noetherian, $J$ is
finitely generated, say by the vectors
\[
(r_0^{(j)},\dots,r_{d-1}^{(j)}) \in A_{n-1}^d,
\qquad j=1,\dots,N.
\]
Lift each generator to an element
\[
r^{(j)}(z',w)
:= r_0^{(j)}(z') + \cdots + r_{d-1}^{(j)}(z') w^{d-1}
\in I'.
\]

By construction, every element $r\in I'$ is an $A_{n-1}$--linear
combination of the $r^{(j)}$, and hence also an $A_n$--linear combination.
Thus
\[
I = (P, r^{(1)},\dots,r^{(N)}),
\]
so $I$ is finitely generated. Since $I$ was arbitrary, $A_n$ is Noetherian.
\end{proof}

\begin{theorem}
The ring $A_n = \C\{z_1,\dots,z_n\}$ is a unique factorization domain.
\end{theorem}

\begin{proof}
We proceed by induction on $n$. For $n=1$, the ring $\C\{z\}$ is a
discrete valuation ring, hence a UFD.

Assume the result holds for $A_{n-1}$. Every nonunit $f\in A_n =
A_{n-1}\{w\}$ can be made regular in $w$ after a linear change of
coordinates. By the Weierstrass Preparation Theorem, we may write
\[
f = u \cdot P,
\]
where $u$ is a unit in $A_n$ and $P$ is a \emph{monic Weierstrass
polynomial}
\[
P(w) = w^d + a_{d-1}(z') w^{d-1} + \cdots + a_0(z') \in A_{n-1}[w],
\qquad a_i(0)=0.
\]
Thus factorization questions for $A_n$ reduce to factorization questions
for monic polynomials in $A_{n-1}[w]$.

Since $A_{n-1}$ is a UFD by induction, the polynomial ring
$A_{n-1}[w]$ is also a UFD. Hence every monic polynomial $P$ admits a
unique factorization
\[
P = P_1 \cdots P_r
\]
into monic irreducible polynomials $P_i \in A_{n-1}[w]$.

We claim that these irreducible factors $P_i$ remain irreducible in
$A_n$. Indeed, suppose $P_i = QR$ in $A_n$. By the Weierstrass Division
Theorem, dividing $Q$ by $P_i$ in the variable $w$ gives remainder zero
(since $P_i$ is monic), hence $Q$ lies in $A_{n-1}[w]$. Likewise for $R$.
Thus any factorization of $P_i$ in $A_n$ already occurs in
$A_{n-1}[w]$, contradicting its irreducibility there. Hence $P_i$ is
irreducible in $A_n$, and being monic it is also prime.

Therefore every nonunit $f\in A_n$ factors as
\[
f = u \cdot P_1\cdots P_r,
\]
with $u$ a unit and the $P_i$ pairwise nonassociate irreducible elements.
Uniqueness of this factorization follows from the uniqueness of
factorization in the UFD $A_{n-1}[w]$.

Thus $A_n$ is a unique factorization domain.
\end{proof}

\begin{exercise}\label{ex:codim-2-extn}
...
\subsection{Theta functions for an elliptic curve}

Let $\tau\in\mathfrak{H}$ (upper half-plane) and let
\[
L \;=\; \Z + \tau\Z \subset \C, \qquad
E = \C/L
\]
be the corresponding elliptic curve. Put
\[
q := e^{\pi i\tau}.
\]

\begin{definition}
The \emph{Riemann theta function} is the entire function
\[
\vartheta(z\mid\tau)
:= \sum_{n\in\Z}
...
\qquad z\in\C.
\]
It satisfies the quasi-periodicity relations
\begin{align*}
\vartheta(z+1\mid\tau) &= \vartheta(z\mid\tau),\\
\vartheta(z+\tau\mid\tau)
&= \exp\bigl(-\pi i\tau - 2\pi i z\bigr)\,\vartheta(z\mid\tau).
\end{align*}
Thus $\vartheta$ is a holomorphic section of a degree-$1$ line bundle on $E=\C/L$ (the theta line bundle).
\end{definition}

It is often convenient to use the Jacobi theta functions with characteristics.

\begin{definition}[Theta with characteristic; Jacobi theta functions]
For $a,b\in\R$, the \emph{theta function with characteristic}
$\bigl[\begin{smallmatrix}a\\ b\end{smallmatrix}\bigr]$ is
\[
\vartheta\begin{bmatrix}a\\ b\end{bmatrix}(z\mid\tau)
:= \sum_{n\in\Z}
   \exp\bigl(\pi i (n+a)^2\tau
              + 2\pi i (n+a)(z+b)\bigr).
\]

The four classical Jacobi theta functions are
\begin{align*}
\theta_1(z\mid\tau)
&:= \vartheta\!\begin{bmatrix}\tfrac12\\ \tfrac12\end{bmatrix}(z\mid\tau)
 = 2 \sum_{n=0}^\infty
   (-1)^n q^{(n+\tfrac12)^2}
   \sin\bigl((2n+1)\pi z\bigr),\\[0.3em]
\theta_2(z\mid\tau)
&:= \vartheta\!\begin{bmatrix}\tfrac12\\ 0\end{bmatrix}(z\mid\tau)
 = 2 \sum_{n=0}^\infty
   q^{(n+\tfrac12)^2}
   \cos\bigl((2n+1)\pi z\bigr),\\[0.3em]
\theta_3(z\mid\tau)
&:= \vartheta\!\begin{bmatrix}0\\ 0\end{bmatrix}(z\mid\tau)
 = 1 + 2\sum_{n=1}^\infty q^{n^2}\cos(2\pi n z),\\[0.3em]
\theta_4(z\mid\tau)
&:= \vartheta\!\begin{bmatrix}0\\ \tfrac12\end{bmatrix}(z\mid\tau)
 = 1 + 2\sum_{n=1}^\infty (-1)^n q^{n^2}\cos(2\pi n z).
\end{align*}

In particular, the genus-$1$ Riemann theta function above is
$\vartheta(z\mid\tau)=\theta_3(z\mid\tau)$.
\end{definition}

Each $\theta_j(z\mid\tau)$ satisfies analogous quasi-periodicity
conditions under $z\mapsto z+1$ and $z\mapsto z+\tau$; these
transformation laws encode the factors of automorphy of the corresponding
degree-$1$ line bundles on $E$.
...
We have seen the same picture of branching 
...
Uniqueness being clear on general grounds, we merely need to prove the existence statement;
...
\subsection{Riemann surfaces and field extensions}
This section is devoted to explaining the equivalence of three categories.
\begin{theorem}
...
There exists, then, an additional meromorphic function $f$ on $R$ which separates the sheets of $R$ over $\mathbb{P}^1$, in the following sense: there exists a point $z_0 \in \mathbb{P}^1$ such that $f$ takes $n$ distinct values at the points of $R$ over $z_0$.
\end{proposition}
...
\]
\begin{theorem}
    Let $\pi : R \to \mathbb{P}^1$ be the holomorphic map associated to the meromorphic function $z$.
\begin{enumerate}
    \item $[\mathbb{C}(R):\mathbb{C}(z)] = \deg \pi$.
...
The theorem follows essentially from part~(4) of the previous result; the only missing ingredient, which rounds up the correspondence between Riemann surfaces and their fields of functions, is:
...
\begin{corollary}
A map $\pi : R \to S$ defines a Galois extension on the fields of meromorphic functions if and only if there exists a group $\Gamma$ of automorphisms of $R$, commuting with $\pi$, and acting simply transitively on the fibres $\pi^{-1}(s)$, for a general $s \in S$. Such a map is called a \textbf{Galois cover with group $\Gamma$}.
\end{corollary}
...
\begin{example}[Galois covers]
    \leavevmode
\begin{enumerate}
    \item[(i)] $\mathbb{P}^1 \longrightarrow \mathbb{P}^1$, with $w \longmapsto z = w^3$. The automorphisms are $z \mapsto \zeta z$, where $\zeta$ is any cube root of $1$.

    \item[(ii)] $\mathbb{C}/L \longrightarrow \mathbb{P}^1$, with $u \longmapsto \wp(u)$. The non-trivial automorphism is $u \longmapsto -u$. The surface $w^2 = 4z^3 - g_2 z - g_3$ is a Galois cover of the $z$-plane, with Galois group $\mathbb{Z}/2$ and automorphism $w \mapsto -w$.
\end{enumerate}
\end{example}


\section{Coherent sheaves}
Let $X$ be a complex manifold and $\cO_X$ the sheaf of holomorphic functions on $X$.
\begin{definition}
A sheaf of $\cO_X$-modules \(\cF\) is \textbf{coherent} if
\begin{enumerate}
    \item \(\cF\) is locally of finite type: for every $x\in X$ there is a neighborhood U and a surjection
\[
\cO_X^{\oplus n}\big|_U \twoheadrightarrow \cF\big|_U,
\]
    \item for every open set \(U\subseteq X\) and every morphism \(\phi:\cO_X^{\oplus p}\big|_U\to\cF\big|_U\), the kernel $\ker\phi$ is locally of finite type.
\end{enumerate}
\end{definition}

\begin{remark}
    For quasi-coherent sheaves on a locally Noetherian scheme, being locally of finite type is a local/stalkwise condition, and kernels of maps of finite free modules are again finite type automatically (because any submodule of a finitely generated module over a Noetherian ring is finitely generated). Thus, a locally Noetherian scheme, a quasi-coherent sheaf \(\cF\) is coherent iff it is of finite type.
\end{remark}

\begin{example}
    $\cO_X$ is coherent. The condition that $\cO_X$ is locally of finite type is trivial. We need to see that for every map
\[
\phi:\cO_X^{\oplus m} \longrightarrow \cO_X
\]
the kernel $\ker\phi$ should be locally generated by finitely many sections. Equivalently, we can show that \begin{enumerate}
    \item for each $x\in X$, the local ring \(\cO_{X,x}\) is Noetherian. We can do this by identifing \(\cO_{X,x}\) with convergent power series in \(n=\dim X\) variables, which is known to be Noetherian by the Weierstrass division theorem (See Example \ref{ex:germs_noetherian}).
    \item For every homomorphism of free modules over \(\cO_{X,x}\),
\[
\varphi:\cO_{X,x}^{\oplus m}\to \cO_{X,x},
\]
the kernel $\ker\varphi$ is a finitely generated \(\cO_{X,x}\)-module. Over a Noetherian ring, submodules of finitely generated modules are finitely generated. These generators can be lifted holomorphically to a neighborhood of $x$ to give local generators of $\ker\phi$.
\end{enumerate}
\end{example}

\begin{definition}
    An \textbf{invertible sheaf} (or line bundle) on a complex manifold $X$ is a coherent sheaf of $\cO_X$-modules $\cL$ which is locally isomorphic to $\cO_X$, i.e. for every $x \in X$ there exists an open neighborhood $U$ of $x$ such that \(\cL\big|_U \cong \cO_X\big|_U\).
\end{definition}

\begin{example}
    The sheaf of holomorphic sections of a holomorphic line bundle is an invertible sheaf.
\end{example}

\begin{example}
    Let $K$ be the sheaf of holomorphic differentials on a Riemann surface $R$. Then $K$ is an invertible sheaf.
\end{example}
...
\]
So we need $f(1/w)/w^2$ to be holomorphic at $w = 0$, so $f$ should extend holomorphically at $\infty$ and have a double zero there. But then $f$ must be zero.
\end{example}
...
\subsection{Vector bundles and elementary transformations}
...
\end{align*}
where $n_i \geq n_{i+1}$. Moreover, the $n_i$ are uniquely determined by $V$. The degree of $V$ is the sum of the $n_i$.
\end{theorem}


\begin{exercise}
In this exercise we will prove Grothendieck's theorem assuming the fact that every vector bundle on $\mathbb{P}^1$ can be constructed from a trivial bundle by a finite sequence of elementary transformations.
    \begin{enumerate}[(a)]
...
\begin{example}
    On $\P^1$, we have homeomorphic but not biholomorphic vector bundles $\cO(1) \oplus \cO(-1)$ and $\cO \oplus \cO$. They both have degreee zero and the same number of sections. However, in \(\cO\oplus\cO\), the two independent sections give a global holomorphic trivialization whereas in \(\cO(1)\oplus\cO(-1)\), the same-dimensional space of global sections only generates a proper subbundle.
\end{example}

\subsection{Riemann-Roch theorem for vector bundles}
...
sheaf cohomology, and the sequence~\eqref{pp} will be the long exact sequence of 
...
\subsection{Coherent sheaf cohomology}
...
\begin{proof}
    ...
    We will reduce the proof to the case $R = \P^1$. In this case, we have already explicitly verified the isomorphism. We wish to construct an isomorphism of sheaves on $\P^1$
...
\begin{proof}
    $g=0$ case is clear since we have explicit meromorphic functions on $\mathbb{P}^1$ with a single pole of any order at any point. So we assume $g\geq1$. A positive integer $n$ is a non-gap at $p$ if there exists a meromorphic function whose only pole is at $p$ and has order exactly $n$.
    
    For any divisor $D$ and point $p$, $0\leq l(D+p)-l(D)\leq1$, so in particular every $n\geq1$ is either a gap or a non-gap. Also, for $g\geq1$, $1$ is always a gap: a function with a single simple pole at $p$ would give a degree-1 map $X\to\mathbb{P}^1$, so $X\cong\mathbb{P}^1$ (genus 0), contradiction.

Riemann-Roch and Serre duality immediately imply that every $m\geq 2g-1$ is a non-gap. Hence all gaps lie in $\{1,\dots,2g-1\}$.

Let $G(m)$ be the number of gaps in $\{1,\dots,m\}$. The number of non-gaps in that interval are is $m-G(m)$. Starting from $l(0)=1$ and using that each non-gap increases $l$ by 1 while each gap leaves it unchanged, for any $m\geq1$, which implies that $l(mp) = 1 + (m-G(m))$. Now take $m\geq 2g$. Then $l(mp) = m+1-g$ and equating, we get $G(m) = g$ for all $m\geq 2g-1$. 
\end{proof}
The generic sequence of gaps is \begin{align*}
    1, 2, \ldots, g
\end{align*} A point $p$ is called a \textbf{Weierstrass point} if its gap sequence differs from the generic one. For example, on a hyperelliptic curve, the Weierstrass points are the branch points of the degree $2$ map to $\P^1$ and have gap sequence \begin{align*}
    1, 3, 5, \ldots, 2g - 1
\end{align*} There are always finitely many Weierstrass points on a compact Riemann surface of genus $g \geq 2$.


\begin{exercise}
A nodal Riemann surface $S$ is one which is smooth except for finitely many singularities that look locally like
\[
\{(x, y) \mid xy = 0\} \subset \mathbb{C}^2.
\]
It is obtained from a smooth Riemann surface $\widetilde{S}$ by identifying pairs of points; $\widetilde{S} \to S$ is called the normalization. Define the canonical bundle $K_S$ of $S$ to be the sheaf of differentials holomorphic on $\widetilde{S}$, except for simple poles at the nodes, with opposite residues on the two branches of $S$.
...
It turns out once you trivialize at a point $r \in R$, that these individual line bundles on $R$
...
hence we obtain a single valued \textbf{meromorphic function on \(R\)}.
...
\begin{exercise}\label{ex:coh-filtered-colimits}
...
\]
and the result of Exercise \ref{ex:coh-filtered-colimits} to show that the ad hoc definition of cohomology of vector bundles via principal parts computes the genuine sheaf cohomology of vector bundles.
\end{exercise}
...
By the result of Exercise \ref{ex:coh-filtered-colimits}, we have
...
But $\varinjlim_D H^0(R;\mathcal O(V(D)))$ is precisely the space of global meromorphic sections of $V$. Thus,
...
\begin{definition}
We introduce the following spaces of $p$-forms on manifolds.
...
in fact $\cD^p(M)$ is dense in $\cE^p(M)$ and thus non-complete for the induced topology. Spaces of \textbf{currents} are defined as the topological duals of the above spaces, in analogy with the usual definition of distributions.

\begin{definition}
The \textbf{space of currents of dimension $p$} (or degree $m-p$) on $M$
...
For now, we let $E$ be a real or complex vector bundle over a smooth manifold $X$. 
\begin{definition}
A \textbf{connection} as a linear map \begin{align*}
...
\begin{exercise}
Let $R$ be a compact Riemann surface of genus $g$.
...
\end{lemma} 
The proof of the following lemma is given at the end of the solution.
\begin{lemma}\label{lem:cc-rep}
...
\begin{theorem}[Weights and curvature of Hermitian metrics on holomorphic line bundles]
Let $L\to X$ be a holomorphic line bundle on a complex manifold, equipped with a
Hermitian metric $h$.  Choose a local holomorphic frame $e$ of $L$ over an open
set $U\subset X$, and write
\[
h(e,e) = e^{-\phi},
\qquad \phi\in C^\infty(U,\R).
\]
Then $\phi$ is called the \textbf{local weight} of the metric $h$ in the frame
$e$.  The following statements hold:

\begin{enumerate}
\item 
The Chern connection $\nabla$ of $(L,h)$ is given in the frame $e$ by
\[
\nabla e = (\partial\phi)\otimes e.
\]

\item
The curvature of the Chern connection is
\[
F_h \;=\; \partial\bar\partial\phi.
\]
In particular, $F_h$ is a real $(1,1)$--form representing $2\pi i\,c_1(L)$.

\item
If $h_0$ and $h$ are two Hermitian metrics related by
\[
h = e^{-\varphi}\,h_0,
\qquad \varphi\in C^\infty(X,\R),
\]
and if $h_0(e,e)=e^{-\phi_0}$, then $h(e,e)=e^{-(\phi_0+\varphi)}$.  Thus the
weights satisfy $\phi=\phi_0+\varphi$, and the curvatures satisfy
\[
F_h = F_0 + \partial\bar\partial\varphi.
\]

\item
Since $\log h(e,e)=-\phi$, one may also write
\[
F_h = -\,\partial\bar\partial \log h(e,e).
\]
\end{enumerate}
\end{theorem}

\begin{proof}
    Let $e$ be a holomorphic frame. Since $h(e,e)=e^{-\phi}$, metric compatibility determines the connection $1$-form $\theta$ by
    \[
    \partial h(e,e) = h(e,e)\bigl(\theta + \overline{\theta}\bigr),\qquad
    \bar\partial h(e,e) = h(e,e)\,\overline{\theta}.
    \]

    Because $e$ is holomorphic, the $(0,1)$-part of the Chern connection equals $\bar\partial$, hence $\overline{\theta}=\bar\partial\phi$. It follows that $\theta=\partial\phi$, giving
    \[
    \nabla e = \theta\otimes e = (\partial\phi)\otimes e,
    \]
    which proves (1).

    Since the curvature is $F_h=\nabla^2=d\theta$ and $\theta=\partial\phi$ has type $(1,0)$, we obtain
    \[
    F_h=\bar\partial(\partial\phi)=\partial\bar\partial\phi,
    \]
    proving (2). If $h=e^{-\varphi}h_0$, then
    \[
    h(e,e)=e^{-\varphi}h_0(e,e)=e^{-(\phi_0+\varphi)},
    \]
    so $\phi=\phi_0+\varphi$. Applying (2) yields
    \[F_h=\partial\bar\partial\phi=\partial\bar\partial(\phi_0+\varphi)=F_0+\partial\bar\partial\varphi,
    \]
    which proves (3). Finally, since $\log h(e,e)=-\phi$, statement (4) is the same as (2) written as
    \[
    F_h=-\partial\bar\partial\log h(e,e).
    \]
\end{proof}

    \begin{example}\label{ex:tautological-bundle}
...
The reason $\bar{\partial}$ operators are important is that they allow us to define holomorphic structures on complex line bundles. In fact, the following proposition holds for complex vector bundles of any rank, but we state it here for line bundles for simplicity.
\begin{proposition}
Let $L$ be a smooth complex line bundle over a complex manifold $X$.
A holomorphic structure on $L$ is equivalent to the choice of a
$\C$-linear operator
\[
\bar\partial_L : \cE^0(L) \longrightarrow \cE^{0,1}(L)
\]
satisfying:
\begin{enumerate}[(i)]
\item The Leibniz rule: $\bar\partial_L(f s) = (\bar\partial f)\otimes s \;+\; f\,\bar\partial_L s,
\qquad f\in C^\infty(X),\; s\in\cE^0(L) $
\item $\bar\partial_L^2 = 0$.
\end{enumerate}
\end{proposition}

\begin{proof}
Suppose first that $L$ is a holomorphic line bundle.  
Choose a holomorphic trivialization on an open set $U\subset X$ with
holomorphic frame $e$.  
Every smooth section has the form $s=f e$, and we define
\[
\bar\partial_L (s) := (\bar\partial f)\, e.
\]
If $e'=g e$ is another holomorphic frame on $U$, then  
$f' = g^{-1}f$ and $\bar\partial g=0$, so
\[
\bar\partial f' = g^{-1} \bar\partial f.
\]
Thus $(\bar\partial f')e' = (\bar\partial f)e$, so the definition
glues on overlaps.  
The resulting operator clearly satisfies the Leibniz rule, and
$\bar\partial_L^2=0$ because in a holomorphic frame it is just the usual
$\bar\partial$ on functions.

Conversely, suppose we are given a $\C$-linear operator
$\bar\partial_L$ satisfying (i)-(ii).  
On a trivializing open set $U$, pick a nowhere-vanishing smooth frame
$e$, and write any section as $s=f e$.  
By the Leibniz rule there is a unique $(0,1)$-form $\alpha$ on $U$ such
that
\[
\bar\partial_L e = \alpha\otimes e.
\]
Then
\[
\bar\partial_L(f e) = (\bar\partial f + \alpha f)\, e.
\]
Applying $\bar\partial_L^2=0$ to $e$ gives $\bar\partial\alpha=0$.
By the $\bar\partial$-Poincaré lemma, on a smaller $U$ there exists a
smooth function $\phi$ with $\alpha=\bar\partial\phi$.
Define a new frame $e' := e\, e^{-\phi}$; then
\[
\bar\partial_L e' = 0.
\]
Thus $e'$ is a $\bar\partial_L$-holomorphic frame, meaning that a
section $s=f e'$ satisfies $\bar\partial_L s=0$ iff $\bar\partial f=0$.
Hence the sheaf
\[
\cO_L(U) := \{\, s\in\cE^0(L|_U) \mid \bar\partial_L s = 0 \,\}
\]
is identified with the usual sheaf of holomorphic functions on $U$.
On overlaps $U\cap V$, if $e'$ and $e''$ are such holomorphic frames,
the transition function $g$ defined by $e'' = g e'$ satisfies
\[
0 = \bar\partial_L e'' = (\bar\partial g)\, e' ,
\]
so $\bar\partial g = 0$; hence $g$ is holomorphic.
Therefore these frames define a holomorphic line bundle structure on $L$.
\end{proof} 
...
For vector bundles, the same applies with an
...
\begin{theorem}[Levi-Civita]
    There exists a unique torsion-free connection $\nabla$ on the tangent bundle $TX$ of a smooth manifold $X$ which is compatible with a Riemannian metric $g$ on $X$, i.e. for any vector fields $X,Y,Z$ on $X$, we have \begin{align*}
        X g(Y,Z) & = g(\nabla_X Y, Z) + g(Y, \nabla_X Z)
    \end{align*} This connection is called the \textbf{Levi-Civita connection}.
\end{theorem}
...
\begin{example}
    Let $V = \C^r$ and equip $V$ with a hermitian inner product $\langle \cdot, \cdot \rangle$, i.e. one which is conjugate linear in the first argument and linear in the second argument. Then the Levi-Civita connection is the unique connection $\nabla$ on the tangent bundle $TV \cong V \times V$ compatible with the Riemannian metric $g$ defined by \begin{align*}
        g(u,v) & = \mathrm{Re} \langle u, v \rangle
    \end{align*} for $u,v \in V$. In this case, the Levi-Civita connection is simply the trivial connection \begin{align*}
        \nabla_{\frac{\partial}{\partial x_i}} \frac{\partial}{\partial x_j} = 0,\qquad
\nabla_{\frac{\partial}{\partial z_i}} \frac{\partial}{\partial z_j} = 0
    \end{align*} 
    Equivalently, to covariantly differentiate the vector field $Y$ in the direction of $X$ at a point $x$, just take the directional derivative of the vector-valued function $Y$ in the direction $X(x)$. \[(\nabla_X Y)(x) = dY_x\big(X(x)\big)\]
In particular all constant vector fields are parallel and all coordinate functions have zero second derivatives. It has zero curvature via a long computation which can be done in coordinates.

In standard coordinates on $V$ we can write
\[
h \;=\; \sum_{i,j} h_{i j}\,d \bar{z_i} \otimes d z_j
\]
with constants $h_{ij}$ (because we chose a constant Hermitian inner product).

The associated Riemannian metric is $g=\Re(h)$, and the associated Kähler form is defined by
\[
\omega(X,Y)\;:=\; g(X,JY),
\]
where $J$ is the standard complex structure. In coordinates one has
\[
\omega \;=\; \frac{i}{2}\sum_{i,j} h_{i j}\,dz_i\wedge d\bar z_j,
\]
and again all coefficients $h_{i j}$ are constant functions on $V$.

Since $\omega$ has constant coefficients in these linear coordinates, $d\omega=0$ because $d$ only differentiates the coefficients, which are constant. Hence $(V,J,g)$ is Kähler.
\end{example}
...
The key construction is the inner product on the space of complex-valued differential forms on a compact Kahler manifold, which allows us to define formal adjoints of the operators $\partial$ and $\bar{\partial}$. This leads to the definition of Laplacians and harmonic forms.
...
\begin{definition}
    ...
    \begin{align*}
        \Delta & = d d^* + d^* d = (d + d^*)^2 \\
        \square & = \partial \partial^* + \partial^* \partial = (\partial + \partial^*)^2 \\
        \bar\square & = \bar{\partial} \bar{\partial}^* + \bar{\partial}^* \bar{\partial} = (\bar{\partial} + \bar{\partial}^*)^2 
    \end{align*}
\end{definition}

On a general complex manifold with a Hermitian metric, these Laplacians are different. However, on a K\"ahler manifold, they essentially coincide. 

\begin{theorem}
    On a K\"ahler manifold $X$, the Laplacians satisfy the relations \begin{align*}
...
Every real cohomology class $[\alpha] \in H^1(X, \R)$ on a compact K\"ahler manifold $X$ has a unique harmonic representative $\alpha$ satisfying $\Delta_d \alpha = 0$. Since $\alpha$ is a real $1$-form, we can write it as \begin{align*}
    \alpha & = \alpha^{1,0} + \alpha^{0,1}
\end{align*} where $\alpha^{1,0}$ is a $(1,0)$-form and $\alpha^{0,1}$ is a $(0,1)$-form. Note that $\overline{\alpha^{1,0}} = \alpha^{0,1}$ since $\alpha$ is real. The Dolbeault cohomology class $[\alpha^{0,1}] \in H^{0,1}(X)$ corresponds to the image of $[\alpha]$ under the natural map \begin{align*}
    H^1(X, \R) & \hookrightarrow H^1(X, \C) \to H^{0,1}(X)
\end{align*} induced by the inclusion of sheaves $\R \hookrightarrow \mathcal{O}_X$.

This is because on a Kahler manifold, the three operators 
\begin{align*}
    \Delta_d & = d d^* + d^* d \\
    \square & = \partial \partial^* + \partial^* \partial \\
    \bar\square & = \bar{\partial} \bar{\partial}^* + \bar{\partial}^* \bar{\partial}
\end{align*} mapping $\cE^{p,q}(X) \to \cE^{p,q}(X)$ are related by \begin{align*}
    \Delta_d & = 2 \bar\square = 2 \square
\end{align*}
In particular, an $\alpha\in\cE^{p,q}$ is $\Delta_d$-harmonic if and only if it is $\bar\square$-harmonic if and only if it is $\square$-harmonic.

Therefore, the map induced by $\R_X \hookrightarrow \mathcal{O}_X$ 
\begin{align*}
    H^1(X, \R_X) & \to H^1(X,\cO_X) \cong H^{0,1}(X) \\
    [\alpha] & \mapsto [\alpha^{0,1}]
\end{align*} is an isomorphism of $\R$-vector spaces.
\end{remark}
...
of Dolbeault representatives. Using the relation to harmonic forms, explain how this relates to
...
The \textbf{Dolbeault} definition of the pairing is
\[
...
\]
This is precisely the \textbf{principal parts} definition of the Serre pairing.
...
geometry, this induces a flat Kahler metric on $A$.  
  \begin{enumerate}
    \item The Dolbeault Laplacian $\bar\square$ on $(0,q)$-forms is translation invariant.  On a flat torus, a $(0,q)$-form is harmonic iff it
...
\item Consider the trivial line bundle
\[
...
= A\times A^\vee
\]
is therefore a holomorphic line bundle, the \textbf{Poincar\'e bundle}.  The
...
\]
A holomorphic line bundle on $A$ with zero Chern class is
...
\begin{remark}
    Let $L$ be a lattice in a complex vector space $V$, and let
$A=V/L$ be the corresponding complex torus. We will verify in detail the fact that a holomorphic line bundle on $A$ with zero Chern class is classified precisely by such a character $L\to U(1)$, and therefore the moduli space of such line bundles is identified with
\[A^\vee = \overline{V}^\vee / L^\vee.
\]

  A unitary character is a homomorphism $\chi: L \longrightarrow U(1)$. Any $\chi$ can be written as $\chi(\ell)=\exp(2\pi i\,\xi(\ell))$ for some real-valued group homomorphism $\xi: L\to\R$. Conversely, any such $\xi$ defines a character this way. So
  \[
  \Hom(L,U(1)) \cong \Hom(L,\R)/\Hom(L,\Z).
  \]

  But
  \[
  \Hom(L,\R) \cong \Hom_\R(V,\R) \cong \overline{V}^\vee
  \]
  where $\Hom_\R(V,\R)$ has a canonical complex structure inherited from that of $V$ by defining $(i\phi)(v) := \phi(iv)$ for $\phi\in \Hom_\R(V,\R)$ and $v\in V$.
  Finally,
  \[
  \Hom(L,\Z)\cong L^\vee.
  \]
...
\subsection{Kodaira Nakano vanishing and the embedding theorem}
We introduce the operators 
\begin{align*}
    L = \omega\wedge - &:\Omega^{p,q}(X)\to \Omega^{p+1,q+1}(X) \\
    \Lambda = L^* &:\Omega^{p,q}(X)\to \Omega^{p-1,q-1}(X) \\
    H = [L,\Lambda] = p+q - n &:\Omega^{p,q}(X)\to \Omega^{p,q}(X)
\end{align*} 
where $\omega$ is the Kähler form on a compact Kähler manifold $X$ of complex dimension $n$. These operators satisfy the commutation relations of the Lie algebra $\mathfrak{sl}_2(\C)$:
\begin{align*}
...
\end{align*}
The Riemannian Hodge decomposition recovers Poincaré duality on $X$. The ordinary Poincaré duality pairing
\[H^k(X;\C)\otimes H^{2n-k}(X;\C)\to \C\]
becomes, via the isomorphism $*:\cH^k \cong \cH^{n-k}$, the $L^2$-pairing. Note that Poincaré duality is topological whereas the Hodge star operator depends on the metric.

\subsection{Dolbeault cohomology for vector bundles}
Let $E\to X$ be a holomorphic vector bundle over a compact complex manifold $X$. The holomorphic structure on $E$ gives rise to the Dolbeault operator
\[
\bar{\partial}_E:\Omega^{p,q}(X,E)\to \Omega^{p,q+1}(X,E)
\]
Put a Hermitian metric $h$ on $E$. We also have the Chern connection $\nabla$ on $E$ compatible with the holomorphic and Hermitian structure, given by the formula \[
\nabla = \partial_E + \bar{\partial}
\] The Hodge decomposition says that $\bar\partial$ cohomology is computed by the kernel of the Dolbeault Laplacian
\[\bar\square = \bar{\partial}\bar{\partial}^* + \bar{\partial}^*\bar{\partial}\]
acting on $\Omega^{p,q}(X,E)$. In particular, on a compact Hermitian manifold,
\[\Omega^{p,q}(X,E)
\;=\;
\ker \square_{\bar\partial}
\;\oplus\;
\operatorname{im}\bar\partial_E
\;\oplus\;
\operatorname{im}\bar\partial_E^*\]
In particular,
$\ker\bar\partial_E = \ker\bar\square \oplus \operatorname{im}\bar\partial_E$. Using the decomposition above, every cohomology class has a unique representative in $\ker\bar\square$, and the map
\[\ker\bar\square \;\longrightarrow\; H^{p,q}_{\bar\partial}(X,E),
\qquad \alpha \longmapsto [\alpha]\]
is an isomorphism.
 
\begin{theorem}
    [Kodaira-Nakano] We have the Weitzenböck formula
    \[
    \bar\square = \square - i[\Lambda,F\wedge ]
    \]
    In particular, $[\Lambda,F\wedge]$ is an order zero differential operator acting on $\Omega^{p,q}(X,E)$.
\end{theorem}



\begin{remark}
    The zeroth order term $-i[\Lambda,F\wedge]$ is self adjoint. For a unitary connection, the curvature $F$ is skew-Hermitian in the
bundle indices, and the commutator $[\Lambda,F\wedge]$ is skew-adjoint. Multiplying a skew-adjoint operator $T$ by $-i$ gives a
self-adjoint operator: $(-iT)^* = iT^* = i(-T) = -iT$.
\end{remark}

\begin{remark}
    $\square$ is semipositive because \begin{align*}
    \langle \square \alpha, \alpha \rangle
    &= \langle (\partial\partial^* + \partial^*\partial)\alpha, \alpha \rangle \\
    &= \langle \partial^*\alpha, \partial^*\alpha \rangle + \langle \partial\alpha, \partial\alpha \rangle \\
    &= \|\partial^*\alpha\|^2 + \|\partial\alpha\|^2 \geq 0
    \end{align*}
\end{remark}

\begin{remark}
    If $-i[\Lambda,F\wedge]$ is positive definite on $\Omega^{p,q}(X,E)$, then $\bar\square$ is strictly positive on $\Omega^{p,q}(X,E)$, hence $\ker\bar\square=0$ and therefore $H^{p,q}_{\bar\partial}(X,E)=0 = \ker\bar\square = 0$.
\end{remark}

\begin{proof}
    By direct computation using the commutation relations above, we have
    \begin{align*}
    \bar\square
    &= [\bar{\partial}, \bar{\partial}^*] \\
    &= [\bar{\partial}, -i[\Lambda,\partial]] \\
    &= -i[[\bar\partial, \Lambda], \partial] - i[\Lambda, [\bar\partial, \partial]] \\
    &= -i[i\partial^*, \partial] - i[\Lambda, F\wedge] \\
    &= \square - i[\Lambda, F\wedge] 
    \end{align*}
\end{proof}

\begin{example}
    Let $E = \cL$ and pick a Hermitian metric $h$ on the holomorphic line bundle $\cL\to X$ with curvature form $F$. We suppose that $F = -2\pi i \,\omega$ for a Kähler form $\omega$. The line bundles which admit such metrics are by definition the positive line bundles. Then
    \begin{align*}
    [\Lambda, F\wedge]
    &= -2\pi i [\Lambda, \omega\wedge] \\
    &= -2\pi i L^* L \\
    &= -2\pi i H \\
    &= -2\pi i (p+q - n)
    \end{align*}
    Therefore, if $p=n$ then $\bar\square = \square + 2\pi (p+q - n) = \square + 2\pi q$ is strictly positive on $\Omega^{n,q}(X,\cL)$ for $q>0$, hence we recover the Kodaira vanishing theorem
    \[
    H^q(X, \cL\otimes \Omega^n_X) = 0
    \quad\text{for } q>0
    \]
    There is also the dual statement
    \[
    H^q(X, \cL^{-1}) = 0
    \quad\text{for } q < n
    \]
...
We notice from this example that positivity seems like a very strong condition on a line bundle. We can relax this condition as follows.
Let $X$ be a compact Kähler manifold.

\begin{definition}
    We say a class $\alpha\in H^{1,1}(X;\R)$ is \textbf{positive} if it is in the real part of $H^{1,1}(X)$ and can be represented by a Kähler form.
\end{definition}

\begin{theorem}
    Suppose $\cL \to X$ is holomorphic line bundle with positive $c_1(\cL)\in H^{1,1}(X;\R)$. Then there exists a Hermitian metric on $\cL$ with curvature $2\pi/i \omega$, where $\omega$ is an integral Kähler form representing $c_1(\cL)$.
\end{theorem}

\begin{remark}
    The constraints $[\omega]\in \Im H^2(X;\Z) \cap H^{1,1}(X;\R)$ and $\omega>0$ imply such Hermitian metric exists on $\cL$. A compact complex $X$ admitting such a form $\omega$ is called a \textbf{Hodge manifold}.
\end{remark}

\begin{theorem}
    [Kodaira embedding theorem] Let $X$ be a Hodge manifold. For a choice of $\cL$, there exists $N>0$ and a holomorphic embedding $X \to \P(H^0(X,\cL^{\otimes N})^\vee)$.
\end{theorem}

An $\cL$ whose power gives such an embedding is called an \textbf{ample line bundle}. If $\cL$ gives an embedding itself, it is called \textbf{very ample}.

\begin{proof}
    [Sketch] We must arrange that there is no base locus, the map is injective, and the map is an immersion. All of these properties are witnessed by the surjectivity of certain evaluation maps. In particular \begin{align*}
    H^0(X,\cL^{\otimes N}) &\to H^0(X,I \cL^{\otimes N}) 
    \end{align*} where $I = \mf{m}_x$ for no base locus, $I = \mf{m}_x \mf{m}_y$ for injectivity, and $I = \mf{m}_x^2$ for immersion. It suffices to demonstrate the vanishing of the cohomology group $H^1(X, I \cL^{\otimes N})$ for sufficiently large $N$. The issue is that when $\dim X > 1$, $I$ is not a line bundle, so we cannot use degree arguments invoking Riemann Roch and Serre duality.

It turns out that you can pass to a blowup $\tilde{X} \to X$ at the finite set of points involved and compute there. In this context, we consider $H^1(\tilde{X}, \pi^* I \cL^{\otimes N})$ where $I = I(E_x), I(E_x + E_y), I(2E_x)$ for the exceptional divisors $E_x, E_y$ over $x,y\in X$. The exceptional divisor, being codimension $1$, puts us back in the line bundle situation and so degree arguments can be used. 
\end{proof}

\begin{example}\label{ex:fubini-study}
...
\[
U_0=\{[1:z_1:\dots:z_n]\}\subset\P^n,
\qquad z=(z_1,\dots,z_n),
\]
and let $\sigma$ denote the standard holomorphic frame of
$\mathcal O_{\P^n}(-1)$ given by the point $(1,z_1,\dots,z_n)\in\C^{n+1}$.
With respect to this frame, the Hermitian metric is
\[
h(\sigma,\sigma)\;=\;1+\sum_{i=1}^n |z_i|^2,
\]
and we computed that the Chern curvature is
\[
F_\nabla
 \;=\;
 \frac{
   -\Bigl(1+\textstyle\sum_{k=1}^n |z_k|^2\Bigr)
      \sum_{i=1}^n dz_i\wedge d\bar z_i
   \;+\;
   \sum_{i,j=1}^n z_i\overline{z_j}\,dz_i\wedge d\bar z_j}
 {(1+\sum_{k=1}^n |z_k|^2)^2}.
\]
Evaluate the associated real $(1,1)$-form on a
tangent vector $v=(v_1,\dots,v_n)\in T^{1,0}_zU_0\cong\C^n$:
\[
\frac{i}{2\pi}F_\nabla(v,\bar v)
 \;=\;
 -\,\frac{1}{2\pi}\cdot
   \frac{
     \bigl(1+\sum_{k=1}^n |z_k|^2\bigr)\,|v|^2
     \;-\;
     |\langle z,v\rangle|^2}
   {(1+\sum_{k=1}^n |z_k|^2)^2}.
\]
...
$|\langle z,v\rangle|^2 \le |z|^2\,|v|^2$, so the numerator satisfies
\[
\bigl(1+\sum |z_k|^2\bigr)\,|v|^2 - |\langle z,v\rangle|^2
 \;\ge\; |v|^2 \;>\;0
\qquad\text{for all } v\neq 0.
\]
Thus the entire expression is strictly negative.  Hence the real
$(1,1)$-form $\frac{i}{2\pi}F_\nabla$ is negative definite, and we conclude that 
$\mathcal O_{\P^n}(-1)$ is negatively curved, while its dual
$\mathcal O_{\P^n}(1)$ is positively curved.
\end{example}

\begin{example}
Positivity is preserved under pullback. If $f:Y\to X$ is a holomorphic map
between compact Kähler manifolds and $\cL\to X$ is a holomorphic line
bundle with a Hermitian metric of positive curvature, then the pullback
bundle $f^*\cL\to Y$ carries the pullback metric whose curvature form is
the pullback of the original curvature form, which is also positive because
if one evaluates the pulled-back form on a tangent vector $v \in T_x^{1,0}X$:
\[(f^*\alpha)(v,\bar v) \;=\; \alpha(df_x v, \overline{df_x v})\]
Since $\alpha$ is positive, the right-hand side satisfies
$\alpha(df_x v, \overline{df_x v}) \ge 0$.


Therefore, from Example \ref{ex:fubini-study}, any ample line bundle $L$ over a compact Kähler manifold $X$ admits a Hermitian metric $h$ whose curvature form
$F$ is a positive $(1,1)$-form by using Kodaira's embedding theorem to
embed $X$ into projective space and pulling back the Fubini-Study metric on
$\cO_{\P^N}(1)$ to $L$.
\end{example}
...
\[
\frac{i}{2\pi}F_0\in\Omega^{1,1}(X,\R)
\] also represents $c_1(L)$, so the difference
\[
...
\bibitem{demailly}
J.-P. Demailly,
\textit{Complex Analytic and Differential Geometry}, lecture notes, 2012.
Available at \url{https://people.math.harvard.edu/~demarco/Math274/Demailly_ComplexAnalyticDiffGeom.pdf}.
    \bibitem{gh} P. Griffiths and J. Harris, Principles of Algebraic Geometry, Wiley-Interscience, 1994.
\bibitem{mumford} D. Mumford, \textit{Abelian Varieties}, Oxford University Press, 1970.
