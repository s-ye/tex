...
\]
the colimit taken over open neighborhoods $U$ of $K$ in $X$.

Thus a section $s\in\mathcal{F}(K)$ is represented by some pair $(U,s_U)$ with $K\subseteq U\subseteq X$ open and $s_U\in\mathcal{F}(U)$.

Using flabbiness, the restriction map $\mathcal{F}(X)\twoheadrightarrow\mathcal{F}(U)$ is surjective, so choose $t\in\mathcal{F}(X)$ with $t|_U=s_U$.
...
\section{Introduction to connections on vector bundles}
...
\section{Hermetian connections, curvature, and Chern classes}
For now, we let $E$ be a real or complex vector bundle over a smooth manifold $X$. Recall that we had introduced a connection as a $\C$-linear map \begin{align*}
