...
-- Create extended columns by concatenating each pair of vectors
extendedColumns = {};
for i from 0 to #columns-1 do (
    for j from 0 to #columns-1 do (
        -- Concatenate the i-th and j-th columns
        concatVector = join(columns#i, columns#j);
        concatVector = append(concatVector, 1);
        extendedColumns = append(extendedColumns, concatVector);
    );
);
